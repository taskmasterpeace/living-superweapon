...
> **Integration status on `codex/playable-integration` (14 September 2026):** The full `codex/audio-all` package is now merged locally, including extra recordings and authoring/source material. The inventory below describes the source handoff, not proof that every cue is wired. Runtime mappings include firearm/scout shots, melee/direct punch impacts, selected steps, weather/domain events, vehicle explosions and confirmed gadget cues; this checkpoint also wires ballistic flesh/metal/stone contacts. Explicit Sound Library choices are preserved. Physical blocked-jab zap, cold-load synth fallbacks and other unassigned loops/phases remain tracked in [issue #25](https://github.com/taskmasterpeace/living-superweapon/issues/25), [issue #18](https://github.com/taskmasterpeace/living-superweapon/issues/18) and `AUDIO_REMAINING_RUNTIME_GAPS_2026-09-14.md`. Check the actual event caller before replacing a mapping.
