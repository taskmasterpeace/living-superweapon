...
Passing data validation is not motion approval. Watch the full clip and loop seam on multiple body sizes, then verify the native contact, interruption and recovery cases above. Current library coverage is the four imported banks plus two native paired-hold previews (hostile grab and friendly carry). The holds use the current Fighter animator, allow holder/receiver selection, and cannot be assigned as imported clips. Their two-second preview interval is not combat duration. Procedural flight and other runtime poses still require separate catalog adapters. Audio audit badges remain unaudited until actual assignments and playback are verified.

## Shared melee approach assignments (2026-09-13)

Character Studio > Model > Fighting motion > Melee approach writes the optional version-one profile field `combat: { groundApproach: 'auto' }`. Allowed values come from `GROUND_APPROACH_FAMILIES` in `src/data/melee-approaches.js`: auto, step, rush, pounce, tackle, bound. Do not copy speeds into profile files; runtime reads the shared family registry. `flight` is deliberately not a ground assignment and this field never grants flight. Auto retains the character's original approach rules. A grounded character retains its assigned family while jumping; a flight-capable character in the air uses its existing aerial rule.

Use `validateProfile`, `profileFromDef`, `applyProfile`, `saveProfile` and existing character-package export/import. Older profiles acquire Auto in the returned value without rewriting stored input. Unsupported families/keys are errors. Local saves take effect on a new game load. Native melee admission, body collision, contact and recovery remain authoritative; assigning a family does not create an animation clip or guarantee damage. The Animation Library still previews source clips/paired holds; rehearse full approaches in the Threat Room.
