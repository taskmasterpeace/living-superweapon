...
The user asked this task to read these answers. Treat them as supplied design direction. The export labels every answer **Draft**; preserve that metadata rather than silently converting it to Locked. This records choices, not implementation completion. The [preserved export](../design/exports/2026-09-10-user-design-decisions.json) is semantically identical to the download, with normalized formatting.
