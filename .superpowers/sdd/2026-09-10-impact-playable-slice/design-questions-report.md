...
- Explicit Cancel / Merge / Replace conflict dialog; Cancel leaves the workbook untouched, Merge warns that imported conflicts overwrite, and Replace warns that omitted entries are removed.
...
GREEN: `node --test tools/design-decisions.test.mjs` — 7 tests passed, 0 failed. These cover the 50-question contract, whitespace-aware answered count, partial-import preservation, replace behavior, notes/status-only conflict detection, malformed/unsafe imports, allowed statuses, field types, and size bounds.
...
- Main review exposed ambiguous native-confirm cancellation that still merged conflicts. Replaced with explicit three-action conflict UI and regression coverage for truly untouched cancellation.
- Desktop active-link navigation could scroll coverage controls away. The question list now scrolls independently beneath pinned coverage and jump controls.

## Verification

- `node --test tools/design-decisions.test.mjs` — PASS, 7/7.
...
- Browser coverage: real typing, status/notes editing, autosave and reload, question jump/navigation, JSON and Markdown downloads, immediate export before autosave, file-input JSON round trip, corrupt import rejection, cancel-without-mutation and explicit conflict merge, storage-write failure with export retained, keyboard focus, desktop pinned progress, and 390px overflow.
