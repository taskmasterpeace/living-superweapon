...
test('new untracked runtime source also participates in build identity',()=>{
 const current={...identity,newSourceHash:'new-model-and-controller'};
 assert.deepEqual(assertSameCheckout(current,{...current}),current);
 assert.throws(()=>assertSameCheckout(current,{...current,newSourceHash:'different'}),/mismatch/);
 assert.throws(()=>assertSameCheckout(current,identity),/mismatch/);
});
