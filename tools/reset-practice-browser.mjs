...
import {beginAcceptance} from './playtest/session.mjs';
...
const {injury=false}=JSON.parse(process.env.PW_PLAYTEST_CONFIG||'{}');
const trial=injury?'defend':'stationary';
...
try{
 ...
 await stageMelee(page,{placement:'reset-console',trial});
 const baseline=await observe(),cycles=[];beginAcceptance(page);
 for(let i=0;i<3;i++){
  if(injury){await page.waitForFunction(()=>PW.game.ms.threatLab.meleeTrial.records.some(r=>r.incoming&&r.healthLost>0),null,{timeout:15000});}
  else{await page.waitForFunction(()=>PW.game.player.slots.lmb.cd<=0,null,{timeout:5000});
  ...
  const before=await observe();if(injury)assert(before.hp<before.maxHp,'Trainer must hurt actual health');else assert(before.ki<before.maxKi-1,'Native beam must spend energy');
  ...
  const after=await observe();assert.equal(after.player,baseline.player);assert.equal(after.ki,after.maxKi);assert.equal(after.hp,after.maxHp);assert.equal(after.stock,baseline.stock);assert.equal(after.manifest,baseline.manifest);assert.equal(after.entities.length,baseline.entities.length);assert(!after.entities.includes(before.target));assert.equal(after.kind,trial);cycles.push({before,after});
 }
 assert.deepEqual(diagnostics.entries,[]);await page.screenshot({path:out+'/reset-console.png'});await saveSnapshot(page,out,{name:'observed-state',errors:diagnostics.entries});await writeFile(out+'/result.json',JSON.stringify({passed:true,injury,cycles,actions:actionHistory(page),errors:diagnostics.entries,staging:'Native menus; one setup positions player facing the reset console and starts the configured trial nearby. Three native beam bursts spend energy, or the defending trainer inflicts actual damage, then E resets each time. Checks player identity, reserve/manifest stability and actor retirement. No outcome injection; KO, carried props and live powers need separate reset cases.'},null,2));
