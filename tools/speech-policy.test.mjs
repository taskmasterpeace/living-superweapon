...
test('near talk, wider yell, hidden enemy and behind-camera admission',()=>{
 const p=speaker(),s=speaker(),g={modeId:'powerworld',running:true,player:p,isFoe:()=>true,canSee:()=>true},point={x:200,y:200};p.pos.x=0;
...
test('leaving PowerWorld retires both local speech and friendly offscreen radio',()=>{
 const p=speaker(),s=speaker(),g={modeId:'powerworld',running:true,player:p,isFoe:()=>false},point={x:200,y:200};
 assert.equal(speechView(g,s,'talk',{},point,800,600).mode,'balloon');
 assert.equal(speechView(g,s,'robot',{radio:true},{...point,behind:true},800,600).mode,'radio');
 g.modeId='freeroam';
 assert.equal(speechView(g,s,'talk',{},point,800,600),null);
 assert.equal(speechView(g,s,'robot',{radio:true},{...point,behind:true},800,600),null);
});
