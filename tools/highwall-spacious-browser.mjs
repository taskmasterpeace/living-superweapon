...
const out='artifacts/highwall-spacious';await mkdir(out,{recursive:true});
...
const report=await page.evaluate(async()=>{
 ...
 const floor=new T.Mesh(new T.BoxGeometry(1200,2,1000),new T.MeshStandardMaterial({color:0x535b4f,roughness:1}));floor.position.y=-1;floor.receiveShadow=true;scene.add(floor);
 const placements=[];const add=(id,moduleId,x,z,extra={})=>placements.push({id,moduleId,x,z,...extra});
 const {highwallLayout}=await import('/src/data/highwall.js');placements.push(...highwallLayout().modules);
 ...
 const camera=new T.OrthographicCamera(-800,800,489,-489,.1,3000);camera.position.set(400,700,850);camera.lookAt(0,14,0);renderer.render(scene,camera);
