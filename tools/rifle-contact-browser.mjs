...
const base=process.env.LSW_BASE_URL||'http://127.0.0.1:5182',authored=process.env.LSW_AUTHORED_RIFLE==='1';
const result={kind:'SUPPLEMENTAL STUDIO POSE INSPECTION — staged camera and actor; not AI gameplay',errors:[],frames:[],shots:[]};
const browser=await chromium.launch({channel:'chromium',headless:process.env.LSW_HEADLESS==='1'}),context=await browser.newContext({viewport:{width:1600,height:1000},recordVideo:{dir:out,size:{width:1600,height:1000}}}),page=await context.newPage();
page.on('pageerror',e=>result.errors.push(e.message));page.on('console',m=>{if(m.type()==='error')result.errors.push(m.text());});
try{
 await page.goto(base+'/studio.html?hero=sarge');await page.waitForFunction(()=>window.STUDIO?.preview?.fighter);await page.getByRole('button',{name:'Pause preview',exact:true}).click();
 result.equipment=authored?'equipment.carbine@1 / imported placeholder':'procedural rifle';
 await page.evaluate(async authored=>{
  ...
  let inspectedGun=f.obj.getObjectByName('weapon-rifle');
  if(authored){
   const {createAuthoredAssetLoader}=await import('/src/engine/authored-assets.js'),{createEquipmentMount}=await import('/src/engine/authored-equipment.js');
   const mount=createEquipmentMount(await createAuthoredAssetLoader().loadEquipmentInstance('equipment.carbine@1'),{weaponKind:'rifle'});
   inspectedGun.visible=false;f.parts.armR.children[2].add(mount);inspectedGun=mount;
  }
  ...
  window.rifleView=side=>{const offset={front:[7,9,24],right:[24,9,1],back:[-7,9,-24],left:[-24,9,1]}[side];p.camera.position.fromArray(offset);p.camera.fov=36;p.controls.target.set(0,5.2,0);p.camera.lookAt(p.controls.target);p.camera.updateProjectionMatrix();p.renderer.render(p.scene,p.camera);
   document.querySelector('.view-tag').textContent=`CLONE RIFLE / ${authored?'IMPORTED PLACEHOLDER / ':''}${side.toUpperCase()} / STAGED INSPECTION`;
  ...
  window.rifleStep=(mode,i)=>{const dt=1/60;g.time+=dt;f.animT+=dt;f.slots.lmb.cd=Math.max(0,f.slots.lmb.cd-dt);f.ki=Math.min(f.maxKi,f.ki+dt*8);f.guarding=mode==='guard';f.poseGuard=f.guarding?1:0;f.vel.set(mode==='idle'?0:8,0,mode==='retreat'?-8:mode==='idle'?0:8);
   ...
   const gun=inspectedGun,at=o=>o.getWorldPosition(new T.Vector3()),muzzle=at(gun.getObjectByName('weapon-muzzle')),shots=[];
