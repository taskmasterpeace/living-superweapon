...
function run(hz,dodge=false,hero='rage',distance=20){const x=mainCombatFixture({mode:'powerworld',hero}),a=x.p,trial=new MeleeTrial(x.g,new THREE.Vector3(0,0,distance)),b=trial.start('retreat');try{
 ...
 for(let i=0;i<hz;i++){const dt=1/hz;x.g.melee.beginContactFrame();x.g.beginBodyContactFrame();a.move(new THREE.Vector3(),dt);b.move(new THREE.Vector3(0,0,dodge?0:1),dt);a.update(dt,x.g);b.update(dt,x.g);x.g.resolveBodies();x.g.melee.endContactFrame();if(a._meleeMotion)assert.ok(a._meleeMotion.point.equals(committed));}
 return 1000-b.hp;
 }finally{x.close();}}
