...
function run(hz,dodge=false,hero='rage',distance=20,startStill=false,delay=0){const x=mainCombatFixture({mode:'powerworld',hero}),a=x.p,trial=new MeleeTrial(x.g,new THREE.Vector3(0,0,distance)),b=trial.start('retreat');try{
 ...
 b.vel.z=startStill?0:30;a.hasAimWorld=true;b.center(a.aimWorld);a.aim3.set(0,0,1);a.faceDir(0,1);b.faceDir(0,-1);
 ...
 for(let i=0;i<hz;i++){const dt=1/hz;x.g.melee.beginContactFrame();x.g.beginBodyContactFrame();a.move(new THREE.Vector3(),dt);b.move(new THREE.Vector3(0,0,dodge||i/hz<delay?0:1),dt);a.update(dt,x.g);b.update(dt,x.g);x.g.resolveBodies();x.g.melee.endContactFrame();if(a._meleeMotion)assert.ok(a._meleeMotion.point.equals(committed));}
 return 1000-b.hp;
 }finally{x.close();}}
