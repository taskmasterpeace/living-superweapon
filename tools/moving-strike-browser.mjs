...
const {scheme='pad'}=JSON.parse(process.env.PW_PLAYTEST_CONFIG||'{}');
...
try{
 ...
 if(scheme==='touch'){await page.getByRole('button',{name:'Select character',exact:true}).tap();await page.getByRole('button',{name:'Enter with squad',exact:true}).tap();}else{await page.keyboard.press('Enter');await page.getByRole('button',{name:'Enter with squad',exact:true}).click();}
 ...
 await performAction(page,'strike',{scheme,until:'melee-charged',move:[1,0]});
 ...
 const actions=actionHistory(page);assert(actions[0].scheme===scheme&&actions[0].movementReleased&&actions[0].released&&actions[0].chargeAtRelease>=.6);
 ...
 await writeFile(out+'/result.json',JSON.stringify({passed:true,scheme,displacement,frames,actions,errors:diagnostics.entries,staging:'Native menus (touch taps in touch mode); stationary Threat Room fixture and initial position/camera staged once. Raw emulated controller or browser touch stick and strike go through native polling. Read-only frame observations prove movement during charge; this does not assert target contact or physical controller support.'},null,2));
