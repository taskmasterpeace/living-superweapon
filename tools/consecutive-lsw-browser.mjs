...
const out='artifacts/marketing/consecutive-lsw';await mkdir(out,{recursive:true});
...
try{
 ...
 await page.goto('http://127.0.0.1:5182/powerworld.html');await page.bringToFront();await page.locator('#hSelect.on').waitFor();await page.waitForTimeout(1200);await page.keyboard.press('Enter');await page.getByRole('button',{name:'LSW SIDE',exact:true}).click();
 ...
 if(receipt.save.awarded.length!==attempt+1)throw Error('Award count mismatch');
 ...
 await page.screenshot({path:out+'/result.png'}).catch(()=>{});await writeFile(out+'/result.json',JSON.stringify({kind:'Normal menus and native LSW G/W, camera heading steered by harness. No position staging, damage overrides or outcome injection.',failure,operations,result,trace,errors},null,2));await context.close();await browser.close();
}
