...
import {ContactProjection} from './cloth-contact-solver.mjs';
const AXES=['x','y','z'];
...
export class RagdollCape {
  constructor(parts,velocity){
    ...
    this.projection=new ContactProjection();this.constraintRow=new Float64Array(9);
    ...
    this.bodies=meshes.map(mesh=>{
      mesh.geometry.computeBoundingBox();return {mesh,matrix:mesh.matrixWorld,lastMatrix:new THREE.Matrix4(),box:mesh.geometry.boundingBox.clone().expandByScalar(.025),worldBox:new THREE.Box3(),inverse:new THREE.Matrix4()};
    });
  ...
  resolveContact(p,position,normal,surfaceMove=false){
    ...
    if(correction>1e-16&&!surfaceMove)contactVelocity(this.velocity,p.pos,normals,offsets,p.contactCount||0,refs);
    p.prev.copy(position).sub(this.velocity);
    if(!surfaceMove&&!this.applyingSurface)for(let i=0;i<(p.contactCount||0);i++)if(refs[i])this.projectSurfaceVelocity(refs[i].face);
    return true;
  }
  contactPosition(p,position,normal,out){
    ...
    let conflict=false;if(!this.applyingSurface)for(let i=0;i<count;i++)if(normals[i].dot(position)<offsets[i]-1e-10){conflict=true;break;}
    if(conflict){
  ...
  refreshContacts(p){
    for(let i=0;i<(p.contactCount||0);i++){
      ...
      let value=ref.offset,weight=0;
      for(let k=0;k<3;k++)if(ref.face[k]===p)weight=ref.weights[k];else value-=ref.weights[k]*ref.normal.dot(ref.face[k].pos);
      p.contactOffsets[i]=value/weight;
    }
  }
  recordSurfaceContact(face,weights,normal,offset){
    ...
    for(const p of face){
      for(let i=0;i<(p.contactCount||0);i++){
        const old=p.contactRefs?.[i];
        if(old&&old.face===face&&old.weights===weights&&old.normal.dot(normal)>1-1e-10){ref=old;break;}
      }
      if(ref)break;
    }
    if(ref){ref.offset=Math.max(ref.offset,offset);this.projectSurfaceVelocity(face);return;}
    ...
    for(let k=0;k<3;k++){
      const p=face[k];if(p.pin||!weights[k])continue;
      const count=p.contactCount||0;(p.contactNormals[count]||(p.contactNormals[count]=new THREE.Vector3())).copy(normal);
      p.contactRefs[count]=ref;p.contactCount=count+1;this.refreshContacts(p);
    }
    this.projectSurfaceVelocity(face);
  }
  addSurfaceConstraints(face,velocity=false){
    const q=this.projection,row=this.constraintRow,stamp=++this.coverStamp;
    for(let k=0;k<3;k++){
      const p=face[k];if(p.pin)continue;
      for(let i=0;i<(p.contactCount||0);i++){
        const ref=p.contactRefs?.[i];
        if(ref){
          if(ref.stamp===stamp)continue;ref.stamp=stamp;
          let slack=-ref.offset,rhs=velocity?0:ref.offset;row.fill(0);
          for(let j=0;j<3;j++){
            const other=ref.face[j],weight=ref.weights[j],index=face.indexOf(other),n=ref.normal;
            slack+=weight*n.dot(other.pos);
            rhs-=weight*(velocity?n.dot(this.velocity.subVectors(other.pos,other.prev)):n.dot(other.pos));
            if(index>=0&&!other.pin){row[index*3]+=weight*n.x;row[index*3+1]+=weight*n.y;row[index*3+2]+=weight*n.z;}
          }
          if(!velocity||slack<1e-8)q.add(row,rhs);
        }else{
          const n=p.contactNormals[i],slack=n.dot(p.pos)-p.contactOffsets[i];
          if(velocity&&slack>=1e-8)continue;
          row.fill(0);row[k*3]=n.x;row[k*3+1]=n.y;row[k*3+2]=n.z;
          q.add(row,velocity?-n.dot(this.velocity.subVectors(p.pos,p.prev)):-slack);
        }
      }
    }
  }
  projectSurfaceVelocity(face){
    const q=this.projection;q.clear();this.addSurfaceConstraints(face,true);
    if(q.solve())for(let k=0;k<3;k++)if(!face[k].pin){
      this.velocity.fromArray(q.solution,k*3);face[k].prev.sub(this.velocity);
    }
  }
  safeSurfaceTargets(face,targets){
    const stamp=++this.coverStamp;
    for(const p of face)for(let i=0;i<(p.contactCount||0);i++){
      const ref=p.contactRefs?.[i];if(!ref||ref.stamp===stamp)continue;ref.stamp=stamp;
      let value=0;
      for(let k=0;k<3;k++){const index=face.indexOf(ref.face[k]);value+=ref.weights[k]*ref.normal.dot(index<0?ref.face[k].pos:targets[index]);}
      if(value<ref.offset-1e-8)return false;
    }
    return true;
  }
      ...
      for(const body of this.bodies){
        ...
        for(const weights of SURFACE_WEIGHTS){
          ...
          for(const key of AXES)for(const sign of [-1,1]){
            ...
            if(distance<best){best=distance;bestOffset=this.proposalNormal.dot(this.candidate);this.contactNormal.copy(this.proposalNormal);for(let i=0;i<3;i++)this.faceTargets[i].copy(this.proposalTargets[i]);}
          }
          if(Number.isFinite(best)){
            this.applyingSurface=true;
            for(let i=0;i<3;i++)if(!face[i].pin)this.resolveContact(face[i],this.faceTargets[i],this.contactNormal,true);
            this.applyingSurface=false;
            this.recordSurfaceContact(face,weights,this.contactNormal,bestOffset);
          }
        }
        this.triBox.makeEmpty();for(const p of face)this.triBox.expandByPoint(p.pos);
      }
  ...
  surfaceTargets(face,weights,normal,depth,world){
    const q=this.projection,row=this.constraintRow;q.clear();this.addSurfaceConstraints(face);
    for(let i=0;i<3;i++)if(!face[i].pin){row.fill(0);row[i*3+1]=1;q.add(row,(world?.heightAt?.(face[i].pos.x,face[i].pos.z)||0)+.035-face[i].pos.y);}
    row.fill(0);for(let i=0;i<3;i++)if(!face[i].pin){row[i*3]=weights[i]*normal.x;row[i*3+1]=weights[i]*normal.y;row[i*3+2]=weights[i]*normal.z;}q.add(row,depth);
    if(!q.solve())return Infinity;
    let cost=0;
    for(let i=0;i<3;i++){
      const p=face[i],target=this.proposalTargets[i];
      target.fromArray(q.solution,i*3).add(p.pos);
  ...
  coverTargets(face,normal,offset,world){
    const q=this.projection,row=this.constraintRow;q.clear();this.addSurfaceConstraints(face);
    for(let i=0;i<3;i++){
      const p=face[i];row.fill(0);
      if(!p.pin){row[i*3]=normal.x;row[i*3+1]=normal.y;row[i*3+2]=normal.z;}
      q.add(row,offset-normal.dot(p.pos));
      if(!p.pin){row.fill(0);row[i*3+1]=1;q.add(row,(world?.heightAt?.(p.pos.x,p.pos.z)||0)+.035-p.pos.y);}
    }
    if(!q.solve())return Infinity;
    let cost=0;
    for(let i=0;i<3;i++){
      const target=this.proposalTargets[i],p=face[i];target.fromArray(q.solution,i*3).add(p.pos);
      if(!p.pin&&target.y<(world?.heightAt?.(target.x,target.z)||0)+.035-1e-8)return Infinity;
      cost+=target.distanceToSquared(p.pos);
    }
    return this.safeSurfaceTargets(face,this.proposalTargets)&&this.safeCoverTargets(face,this.proposalTargets)?cost:Infinity;
  ...
  collideCoverSurface(world){
    ...
    for(const face of this.faces){
      ...
      for(const body of this.covers){
        ...
        let best=Infinity,bestAxis=-1,bestSign=0;
        for(let axis=0;axis<3;axis++)for(const sign of [-1,1]){
          if(entryMask&&!(entryMask&(1<<(axis*2+(sign>0?1:0)))))continue;
          const key=AXES[axis],value=(sign<0?body.box.min[key]:body.box.max[key])+sign*.003;
          const e=body.inverse.elements;this.proposalNormal.set(e[axis],e[axis+4],e[axis+8]).multiplyScalar(sign).normalize();
          this.candidate.set(0,0,0);this.candidate[key]=value;this.candidate.applyMatrix4(body.matrix);
          const cost=this.coverTargets(face,this.proposalNormal,this.proposalNormal.dot(this.candidate),world);
          if(cost<best){best=cost;bestAxis=axis;bestSign=sign;for(let i=0;i<3;i++)this.faceTargets[i].copy(this.proposalTargets[i]);}
        }
        ...
        this.applyingSurface=false;this.projectSurfaceVelocity(face);
        this.triBox.makeEmpty();for(const p of face)this.triBox.expandByPoint(p.pos);
      }
    }
...
function contactVelocity(v,position,normals,offsets,count,refs){
  const vx=v.x,vy=v.y,vz=v.z;let best=v.lengthSq(),bx=0,by=0,bz=0;
  const active=i=>!refs?.[i]&&normals[i].dot(position)-offsets[i]<1e-8;
  const consider=(x,y,z)=>{
