// Rejected production candidate: isolated contact math passes; full falls regress.
...
export class RagdollCape {
  ...
  collide(p,world){
    for(const body of this.colliders){
      ...
      if(segmentOutside(p.pos,p.prev,body.worldBox))continue;
      this.local.copy(p.pos).applyMatrix4(body.inverse);this.before.copy(p.prev).applyMatrix4(body.inverse);
      if(body.box.containsPoint(this.local)){
        let best=Infinity;
      ...
      if(projectBox(this.local,this.before,body.box,this.normal)){
    ...
    for(const face of this.faces){
      ...
      for(const body of this.bodies){
        ...
        for(const weights of SURFACE_WEIGHTS){
          ...
          if(Number.isFinite(best))this.recordSurfaceContact(face,weights,this.contactNormal,offset);
        }
        this.triBox.makeEmpty();for(const p of face)this.triBox.expandByPoint(p.pos);
      }
    }
...
function projectBox(point,previous,box,normal){
  if(box.containsPoint(point))return projectEmbedded(point,box,normal);
