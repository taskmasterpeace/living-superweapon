...
export class RagdollCape {
  ...
  commitContact(p,position,normal){
    ...
    if(!this.applyingSurface){
      let known=false;for(let i=0;i<count;i++)if(!refs[i]&&normals[i].dot(normal)>1-1e-10){known=true;if(correction>1e-16)offsets[i]=Math.max(offsets[i],p.pos.dot(normal));break;}
      if(!known&&correction>1e-16){(normals[count]||(normals[count]=new THREE.Vector3())).copy(normal);offsets[count]=p.pos.dot(normal);refs[count]=null;p.contactCount=count+1;}
    }
  ...
  recordSurfaceContact(face,weights,normal,offset){
    ...
    if(ref)ref.offset=Math.max(ref.offset,offset);
      ...
      ref.face=face;ref.weights=weights;ref.normal.copy(normal);ref.offset=offset;
      for(let k=0;k<3;k++){
        const p=face[k];if(p.pin||!weights[k])continue;const i=p.contactCount||0;
        (p.contactNormals[i]||(p.contactNormals[i]=new THREE.Vector3())).copy(normal);p.contactRefs[i]=ref;p.contactCount=i+1;
      }
    }
  ...
  safeSurfaceTargets(points,targets){
    ...
    for(let j=0;j<points.length;j++){
      ...
      for(let i=0;i<(p.contactCount||0);i++){
        ...
        if(!ref){if(p.contactNormals[i].dot(targets[j])<p.contactOffsets[i]-1e-9)return false;continue;}
        let sample=0;
        for(let k=0;k<3;k++){const index=points.indexOf(ref.face[k]);sample+=ref.weights[k]*ref.normal.dot(index<0?ref.face[k].pos:targets[index]);}
