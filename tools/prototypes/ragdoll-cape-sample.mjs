// Experimental weighted contacts on the accepted real-substep static-sweep baseline.
// Not imported by the production game or Studio.
...
export class RagdollCape {
  constructor(parts,velocity){
    ...
    this.surfacePool=[];this.surfaceCount=0;
    this.velocityNormals=[];this.velocityOffsets=[];this.velocityStep=new THREE.Vector3();this.velocityBest=new THREE.Vector3();
    this.positionNormals=[];this.positionOffsets=[];
    this.supportWorld=new THREE.Vector3();this.supportLocal=new THREE.Vector3();this.supportProposal=new THREE.Vector3();
    ...
    for(let i=0;i<a.count;i++){
      ...
      this.points.push({local,pos,prev:pos.clone().addScaledVector(velocity,-this.dt),pin:i<this.columns,target:pos.clone(),last:pos.clone(),contactNormals:[],contactOffsets:[],contactRefs:[],contactCount:0,faces:[]});
    }
    ...
    for(let i=this.columns;i<this.points.length;i++){
      const p=this.points[i],anchor=this.points[i%this.columns];
      p.tether={anchor,length:Math.max(bind[i].distanceTo(bind[i%this.columns]),p.pos.distanceTo(anchor.pos))*1.05};
    }
  ...
  resolveContact(p,position,normal){
    ...
    return this.commitContact(p,this.projected,normal);
  }
  commitContact(p,position,normal){
    const normals=p.contactNormals||(p.contactNormals=[]),offsets=p.contactOffsets||(p.contactOffsets=[]),count=p.contactCount||0;
    const refs=p.contactRefs||(p.contactRefs=[]);
    const bodies=p.contactBodies||(p.contactBodies=[]);
    ...
    if(!this.applyingSurface){
      let known=false;for(let i=0;i<count;i++)if(!refs[i]&&bodies[i]===(this.contactBody||null)&&normals[i].dot(normal)>1-1e-10){known=true;if(correction>1e-16)offsets[i]=Math.max(offsets[i],p.pos.dot(normal));break;}
      if(!known&&correction>1e-16){(normals[count]||(normals[count]=new THREE.Vector3())).copy(normal);offsets[count]=p.pos.dot(normal);refs[count]=null;bodies[count]=this.contactBody||null;p.contactCount=count+1;}
    }
    p.prev.copy(position).sub(this.velocity);
    if(correction>1e-16&&!this.applyingSurface&&!this.committingFace)this.stabilizeVelocity(p);
    return true;
  }
  contactPosition(p,position,normal,out){
    const count=this.positionConstraints(p,position),normals=this.positionNormals,offsets=this.positionOffsets;
    ...
    if(!this.withinReach(p,position))return false;
  ...
  contactSupported(p,index,position){
    const ref=p.contactRefs?.[index],body=ref?.body||p.contactBodies?.[index];
    if(!body)return true;
    if(ref){
      this.supportWorld.set(0,0,0);
      for(let i=0;i<3;i++)this.supportWorld.addScaledVector(ref.face[i]===p?position:ref.face[i].pos,ref.weights[i]);
    }else this.supportWorld.copy(position);
    return body.box.containsPoint(this.supportLocal.copy(this.supportWorld).applyMatrix4(body.inverse));
  }
  positionConstraints(p,position){
    this.refreshContacts(p);let count=0;
    for(let i=0;i<(p.contactCount||0);i++)if(this.contactSupported(p,i,position)){
      this.positionNormals[count]=p.contactNormals[i];this.positionOffsets[count++]=p.contactOffsets[i];
    }
    return count;
  }
  withinReach(p,position){
    return !this.started||!p.tether||position.distanceToSquared(p.tether.anchor.pos)<=p.tether.length*p.tether.length+1e-10;
  }
  constrainReach(){
    for(const p of this.points)if(p.tether){
      const {anchor,length}=p.tether;this.delta.subVectors(p.pos,anchor.pos);const distance=this.delta.length();
      if(distance<=length)continue;
      this.velocity.subVectors(p.pos,p.prev);this.delta.multiplyScalar(1/distance);
      p.pos.copy(anchor.pos).addScaledVector(this.delta,length);
      // Remove only velocity trying to stretch the fabric beyond its anchor.
      this.velocity.addScaledVector(this.delta,-Math.max(0,this.velocity.dot(this.delta)));
      p.prev.copy(p.pos).sub(this.velocity);
    }
  }
  refreshContacts(p){
    // A sample plane constrains a weighted sum, not three fixed corner planes.
    // Derive this corner's remaining clearance from its CURRENT neighbors.
    for(let i=0;i<(p.contactCount||0);i++){
      const ref=p.contactRefs?.[i];if(!ref)continue;
      let offset=ref.offset,weight=0;
      for(let k=0;k<3;k++)if(ref.face[k]===p)weight=ref.weights[k];else offset-=ref.weights[k]*ref.normal.dot(ref.face[k].pos);
      p.contactOffsets[i]=offset/weight;
    }
  }
  recordSurfaceContact(face,weights,normal,offset){
    let ref;
    for(const p of face){
      for(let i=0;i<(p.contactCount||0);i++){
        const old=p.contactRefs?.[i];if(old&&old.face===face&&old.weights===weights&&old.normal.dot(normal)>1-1e-10){ref=old;break;}
      }
      if(ref)break;
    }
    if(ref)ref.offset=Math.max(ref.offset,offset);
    else{
      ref=this.surfacePool[this.surfaceCount]||(this.surfacePool[this.surfaceCount]={normal:new THREE.Vector3()});this.surfaceCount++;
      ref.face=face;ref.weights=weights;ref.normal.copy(normal);ref.offset=offset;ref.body=this.contactBody||null;
      for(let k=0;k<3;k++){
        const p=face[k];if(p.pin||!weights[k])continue;const i=p.contactCount||0;
        (p.contactNormals[i]||(p.contactNormals[i]=new THREE.Vector3())).copy(normal);p.contactRefs[i]=ref;(p.contactBodies||(p.contactBodies=[]))[i]=null;p.contactCount=i+1;
      }
    }
    // Remove inward SAMPLE velocity. Opposing corner velocities may be a valid
    // rotating/sliding cloth face and must not each be clamped to zero.
    let speed=0,minWeight=Infinity;
    for(let k=0;k<3;k++){const p=face[k],w=weights[k];speed+=w*(normal.dot(p.pos)-normal.dot(p.prev));if(!p.pin&&w>0)minWeight=Math.min(minWeight,w);}
    if(speed>=-1e-10||!Number.isFinite(minWeight))return;
    // Use compatible velocity rays BEFORE applying the impulse. The same
    // bounded allocation as the position solve preserves neighboring samples.
    const maximum=-speed/minWeight;let effectiveMass=0;
    for(let k=0;k<3;k++){
      const p=face[k];this.mobility[k]=0;if(p.pin||!weights[k])continue;
      const count=this.velocityConstraints(p,ref);this.velocity.subVectors(p.pos,p.prev);
      const ray=this.proposalTargets[k];
      if(!contactDisplacement(ray,this.velocity,normal,maximum,this.velocityNormals,this.velocityOffsets,count))continue;
      ray.divideScalar(maximum);this.mobility[k]=1/ray.lengthSq();effectiveMass+=weights[k]*weights[k]*this.mobility[k];
    }
    if(effectiveMass>1e-12)for(let k=0;k<3;k++)if(this.mobility[k])face[k].prev.addScaledVector(this.proposalTargets[k],speed*weights[k]*this.mobility[k]/effectiveMass);
  }
  velocityConstraints(p,exclude=null){
    this.refreshContacts(p);this.velocity.subVectors(p.pos,p.prev);
    const normals=this.velocityNormals,offsets=this.velocityOffsets;let count=0;
    for(let i=0;i<(p.contactCount||0);i++){
      const n=p.contactNormals[i];if(n.dot(p.pos)-p.contactOffsets[i]>=1e-8)continue;
      const ref=p.contactRefs?.[i];let offset=0,weight=1;
      if(ref&&ref===exclude)continue;
      if(ref){
        for(let k=0;k<3;k++)if(ref.face[k]===p)weight=ref.weights[k];else{const q=ref.face[k];offset-=ref.weights[k]*(n.dot(q.pos)-n.dot(q.prev));}
        offset/=weight;
      }
      let found=-1;for(let j=0;j<count;j++)if(normals[j].dot(n)>1-1e-10){found=j;break;}
      if(found>=0)offsets[found]=Math.max(offsets[found],offset);
      else{(normals[count]||(normals[count]=new THREE.Vector3())).copy(n);offsets[count++]=offset;}
    }
    return count;
  }
  stabilizeVelocity(p){
    const count=this.velocityConstraints(p),normals=this.velocityNormals,offsets=this.velocityOffsets;
    let valid=true;for(let i=0;i<count;i++)if(normals[i].dot(this.velocity)<offsets[i]-1e-10){valid=false;break;}
    if(valid)return;
    // Exact small 3-D projection. Reuse the existing active-plane solver; no
    // nine-coordinate iterative optimization for every face/body candidate.
    let best=Infinity;
    for(let i=0;i<count;i++)if(contactDisplacement(this.velocityStep,this.velocity,normals[i],offsets[i]-normals[i].dot(this.velocity),normals,offsets,count)){
      const cost=this.velocityStep.lengthSq();if(cost<best){best=cost;this.velocityBest.copy(this.velocityStep);}
    }
    if(Number.isFinite(best))p.prev.copy(p.pos).sub(this.velocity).sub(this.velocityBest);
  }
  ...
  safeSurfaceTargets(points,targets){
    // Check the complete proposal: individually valid corners may consume
    // the same neighboring sample's clearance. Never partially commit it.
    for(let j=0;j<points.length;j++){
      const p=points[j];
      if(!this.withinReach(p,targets[j]))return false;
      for(let i=0;i<(p.contactCount||0);i++){
        const ref=p.contactRefs?.[i];
        if(!ref){if(this.contactSupported(p,i,targets[j])&&p.contactNormals[i].dot(targets[j])<p.contactOffsets[i]-1e-9)return false;continue;}
        let sample=0;
        this.supportWorld.set(0,0,0);
        for(let k=0;k<3;k++){const index=points.indexOf(ref.face[k]),position=index<0?ref.face[k].pos:targets[index];sample+=ref.weights[k]*ref.normal.dot(position);this.supportWorld.addScaledVector(position,ref.weights[k]);}
        if(ref.body&&!ref.body.box.containsPoint(this.supportLocal.copy(this.supportWorld).applyMatrix4(ref.body.inverse)))continue;
        if(sample<ref.offset-1e-9)return false;
      }
    }
    return true;
  }
      ...
      for(const body of this.bodies){
        ...
        for(const weights of SURFACE_WEIGHTS){
          ...
          for(const key of AXES)for(const sign of [-1,1]){
            ...
            if(distance<best){best=distance;offset=this.proposalNormal.dot(this.candidate);this.contactNormal.copy(this.proposalNormal);for(let i=0;i<3;i++)this.faceTargets[i].copy(this.proposalTargets[i]);}
          }
          if(Number.isFinite(best)){
            // All corners were validated together. Re-projecting them one by
            // one changes the proposal after validation and can trap a neighbor.
            this.applyingSurface=true;
            for(let i=0;i<3;i++)if(!face[i].pin)this.commitContact(face[i],this.faceTargets[i],this.contactNormal);
            this.applyingSurface=false;
            this.recordSurfaceContact(face,weights,this.contactNormal,offset);
          }
        }
        this.triBox.makeEmpty();for(const p of face)this.triBox.expandByPoint(p.pos);
      }
  ...
  surfaceTargets(face,weights,normal,depth,world){
    ...
    for(let i=0;i<3;i++){
      ...
      this.supportProposal.copy(p.pos).addScaledVector(normal,maximum);
      const count=this.positionConstraints(p,this.supportProposal);
      if(!contactDisplacement(direction,p.pos,normal,maximum,this.positionNormals,this.positionOffsets,count))continue;
    ...
    return this.safeSurfaceTargets(face,this.proposalTargets)&&this.safeCoverTargets(face,this.proposalTargets)?cost:Infinity;
  }
    ...
    for(const face of this.faces){
      ...
      for(const body of this.covers){
        ...
        for(let axis=0;axis<3;axis++)for(const sign of [-1,1]){
          ...
          for(let i=0;i<3;i++){
            this.proposalTargets[i].copy(face[i].pos);
            ...
            this.proposalTargets[i].copy(this.candidate);
            cost+=this.candidate.distanceToSquared(face[i].pos);
          }
          if(valid&&cost<best&&this.safeSurfaceTargets(face,this.proposalTargets)&&this.safeCoverTargets(face,this.proposalTargets)){
            best=cost;this.contactNormal.copy(this.proposalNormal);for(let i=0;i<3;i++)this.faceTargets[i].copy(this.proposalTargets[i]);
          }
        }
        if(!Number.isFinite(best))continue;
        this.committingFace=true;
        for(let i=0;i<3;i++)if(!face[i].pin){
          correction=Math.max(correction,this.faceTargets[i].distanceToSquared(face[i].pos));
          this.commitContact(face[i],this.faceTargets[i],this.contactNormal);
        }
        this.committingFace=false;
        for(const p of face)if(!p.pin)this.stabilizeVelocity(p);
        this.triBox.makeEmpty();for(const p of face)this.triBox.expandByPoint(p.pos);
      }
    }
    this.contactBody=null;this.solvingCover=false;return correction;
  }
  restore(){
