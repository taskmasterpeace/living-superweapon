...
export class RagdollCape {
  constructor(parts,velocity){
    ...
    this.coverProbe=new THREE.Triangle();this.coverBefore=new THREE.Triangle();this.coverStamp=0;this.surfaceStamp=0;
    this.dt=1/120;this.started=false;this.still=0;this.asleep=false;this.unionContacts=false;
    ...
    this.rayLocal=new THREE.Vector3();this.rayEnd=new THREE.Vector3();
  ...
  update(dt,world){
    ...
    for(let step=0;step<steps;step++){
      ...
      for(let it=0;surfaceCorrection>1e-10&&it<12;it++){
        for(const p of this.points)(p.contactLast||(p.contactLast=new THREE.Vector3())).copy(p.pos);
        for(const p of this.points)if(!p.pin)this.collide(p,world);
        surfaceCorrection=this.collideSurface(world);
        // Only escalate a genuine contact cycle: the pass made corrections
        // but returned every vertex to its starting position. Ordinary local
        // cloth motion keeps its existing, less restrictive contact solve.
        if(!this.unionContacts&&surfaceCorrection>1e-10&&this.hasStalledSurface())this.unionContacts=true;
      }
    }
  ...
  hasStalledSurface(){
    for(const face of this.faces){
      if(face.every(p=>p.pin)||face.some(p=>p.pos.distanceToSquared(p.contactLast)>1e-12))continue;
      for(const weights of SURFACE_WEIGHTS){
        this.sample.set(0,0,0);for(let i=0;i<3;i++)this.sample.addScaledVector(face[i].pos,weights[i]);
        for(const body of this.bodies)if(body.worldBox.containsPoint(this.sample)&&body.box.containsPoint(this.rayLocal.copy(this.sample).applyMatrix4(body.inverse)))return true;
      }
    }
    return false;
  }
  ...
  collideSurface(world){
    this._contactCorrection=0;this.surfaceStamp++;
      ...
      for(const body of this.bodies){
        ...
        for(const weights of SURFACE_WEIGHTS){
          ...
          for(const key of AXES)for(const sign of [-1,1]){
            ...
            const localDepth=this.delta.dot(this.proposalNormal),depth=this.unionContacts?this.surfaceDepth(this.sample,this.proposalNormal,localDepth):localDepth;
            const distance=this.surfaceTargets(face,weights,this.proposalNormal,depth,world);
  ...
  surfaceDepth(origin,normal,depth){
    // A torso/pelvis overlap is one solid union. Exiting the first box into
    // the second creates an endless pair of opposing midpoint corrections.
    // Extend the escape ray through connected body intervals, not through
    // static cover (which retains its swept/incident-triangle safeguards).
    // Most face exits already clear the union. Only intersect all ray slabs
    // when its endpoint actually lies in another body volume.
    this.rayEnd.copy(origin).addScaledVector(normal,depth);
    if(!this.bodies.some(body=>body.worldBox.containsPoint(this.rayEnd)&&body.box.containsPoint(this.rayLocal.copy(this.rayEnd).applyMatrix4(body.inverse))))return depth;
    let count=0;
    for(const body of this.bodies){
      this.rayLocal.copy(origin).applyMatrix4(body.inverse);const e=body.inverse.elements;
      let near=-Infinity,far=Infinity;
      for(let k=0;k<3;k++){
        const key=AXES[k],d=e[k]*normal.x+e[k+4]*normal.y+e[k+8]*normal.z;
        if(Math.abs(d)<1e-10){if(this.rayLocal[key]<body.box.min[key]||this.rayLocal[key]>body.box.max[key]){far=-1;break;}continue;}
        const a=(body.box.min[key]-this.rayLocal[key])/d,b=(body.box.max[key]-this.rayLocal[key])/d;
        near=Math.max(near,Math.min(a,b));far=Math.min(far,Math.max(a,b));
      }
      if(far>=0&&near<=far){this.intervals[count++]=near;this.intervals[count++]=far;}
    }
    let changed=true;
    while(changed){changed=false;for(let i=0;i<count;i+=2)if(this.intervals[i]<=depth&&this.intervals[i+1]>=depth){depth=this.intervals[i+1]+.003;changed=true;}}
    return depth;
  }
  surfaceTargets(face,weights,normal,depth,world){
    ...
    for(let i=0;i<3;i++){
      ...
      if(!contactDisplacement(direction,p.pos,normal,maximum,p.contactNormals,p.contactOffsets,count))continue;
      if(this.unionContacts){
      // Sample toward the available ray, not always toward +X/+Z: the two
      // sides of a heightfield ridge have different supporting tangents.
      // Ground support participates before allocation; actual terrain is
      // checked again below for curvature and heightfield cell boundaries.
      const sx=direction.x<0?-.01:.01,sz=direction.z<0?-.01:.01;
      const terrain=p.terrain||(p.terrain={});
      if(terrain.stamp!==this.surfaceStamp||terrain.world!==world||terrain.x!==p.pos.x||terrain.z!==p.pos.z){
        terrain.stamp=this.surfaceStamp;terrain.world=world;terrain.x=p.pos.x;terrain.z=p.pos.z;terrain.floor=world?.heightAt?.(p.pos.x,p.pos.z)||0;
        terrain.xp=terrain.xm=terrain.zp=terrain.zm=undefined;
      }
      const floor=terrain.floor,xKey=sx<0?'xm':'xp',zKey=sz<0?'zm':'zp';
      const dx=terrain[xKey]??(terrain[xKey]=((world?.heightAt?.(p.pos.x+sx,p.pos.z)||0)-floor)/sx);
      const dz=terrain[zKey]??(terrain[zKey]=((world?.heightAt?.(p.pos.x,p.pos.z+sz)||0)-floor)/sz);
      const support=p.contactNormals[count]||(p.contactNormals[count]=new THREE.Vector3());
      support.set(-dx,1,-dz).normalize();
      p.contactOffsets[count]=support.x*p.pos.x+support.y*(floor+.035)+support.z*p.pos.z;
      if(!contactDisplacement(direction,p.pos,normal,maximum,p.contactNormals,p.contactOffsets,count+1))continue;
      }
    ...
    // Existing contacts can bend the allocated corner rays. Validate their
    // actual weighted endpoint, not only the nominal normal used above.
    if(this.unionContacts){
      this.contactDelta.set(0,0,0);for(let i=0;i<3;i++)this.contactDelta.addScaledVector(this.proposalTargets[i],weights[i]);
      for(const body of this.bodies)if(body.worldBox.containsPoint(this.contactDelta)&&body.box.containsPoint(this.rayLocal.copy(this.contactDelta).applyMatrix4(body.inverse)))return Infinity;
    }
