...
const out='artifacts/marketing/touch-aim-throw-2026-09-12';await mkdir(out,{recursive:true});const b=await chromium.launch({headless:false}),c=await b.newContext({viewport:{width:844,height:390},isMobile:true,hasTouch:true,recordVideo:{dir:out}}),p=await c.newPage(),errors=[];p.on('pageerror',e=>errors.push(e.message));
...
await hold('fly',500);await release();await p.evaluate(()=>{window.throwVictim=PW.game.player.grabbing;});
const label=await p.locator('#touch [data-b=grab]').getAttribute('aria-label');await hold('grab',300);
const throwBox=await p.locator('#touch [data-b=grab]').boundingBox(),finger={id:0,x:throwBox.x+throwBox.width/2,y:throwBox.y+throwBox.height/2};
await client.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[finger,{id:1,x:500,y:150}]});
await client.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[finger,{id:1,x:500,y:215}]});
await p.waitForFunction(()=>PW.game.world._lookPitch<-.35,null,{timeout:3000});
await client.send('Input.dispatchTouchEvent',{type:'touchMove',touchPoints:[finger,{id:1,x:500,y:150}]});
if(!await p.evaluate(()=>!!PW.game.player.grabbing&&PW.game.touch.cur.grab))throw Error('Released before deliberate throw');await p.screenshot({path:out+'/aim.png'});await release();
...
const result=await p.evaluate(()=>({records:PW.game.ms.threatLab.meleeTrial.records,grabbing:!!PW.game.player.grabbing,touchGrab:PW.game.touch.cur.grab,victimHp:throwVictim.hp,pitch:PW.game.world._lookPitch,aimReleased:!PW.game.touch._aim}));await p.screenshot({path:out+'/impact.png'});await writeFile(out+'/result.json',JSON.stringify({label,result,errors},null,2));console.log({label,result,errors});
