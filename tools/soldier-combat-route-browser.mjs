...
const out='artifacts/marketing/soldier-combat-route';await mkdir(out,{recursive:true});
...
async function engage(){
 const target=await page.evaluate(()=>{const g=window.PW.game,p=g.player;if(!p?.alive||g.ms.convoyOperation?.finished)return null;const foes=g.entities.filter(e=>e.alive&&e.def&&!e.isDummy&&g.isFoe(p,e)&&p.pos.distanceTo(e.pos)<170&&g.canSee(p,e)&&(e._vis??1)>.4).sort((a,b)=>p.pos.distanceTo(a.pos)-p.pos.distanceTo(b.pos));return foes[0]?.id??null;});
 if(target===null)return;
 await page.keyboard.up('w');await page.mouse.move(720,450);
 for(let burst=0;burst<6;burst++){
  const valid=await page.evaluate(id=>{const g=window.PW.game,e=g.entities.find(e=>e.id===id);if(!e?.alive||!g.player.alive||!g.canSee(g.player,e))return false;const d=e.pos.clone();d.y+=5;d.sub(g.world.camera.position);g.world._lookYaw=Math.atan2(d.x,d.z);g.world._lookPitch=Math.atan2(d.y,Math.hypot(d.x,d.z));return true;},target);if(!valid)break;
  await page.mouse.down();await page.waitForTimeout(200);await page.mouse.up();
 }
}
async function walk(kind,stop){
 for(let i=0;i<160;i++){await engage();
...
try{
 ...
 await page.keyboard.up('w').catch(()=>{});await page.mouse.up().catch(()=>{});
 const result=await page.evaluate(()=>{const g=window.PW.game,o=g.ms.convoyOperation;return {state:o?.state,events:o?.events,scientist:o?.scientist&&{hp:o.scientist.hp,pos:o.scientist.pos.toArray()},player:{hp:g.player.hp,pos:g.player.pos.toArray(),kills:g.player.kills,damage:g.player.damageDealt,ammo:g.player.slots.lmb.ammo},save:g.campaign.snapshot()};}).catch(()=>null);
 await page.screenshot({path:out+'/result.png'});await writeFile(out+'/result.json',JSON.stringify({kind:'Normal menus and native Soldier E/W, camera heading steered by harness. No position staging, damage overrides or outcome injection.',failure,result,trace,errors},null,2));await context.close();await browser.close();
}
