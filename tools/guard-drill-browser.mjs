...
const {facing='front'}=JSON.parse(process.env.PW_PLAYTEST_CONFIG||'{}');assert(['front','rear'].includes(facing),'Unknown guard facing');
...
try{
 ...
 await page.evaluate(facing=>{const g=PW.game,t=g.ms.threatLab.meleeTrial;t.origin.set(0,0,0);t.start('defend');g.player.pos.set(0,0,14);g.player.vel.set(0,0,0);g.world._lookYaw=facing==='rear'?0:Math.PI;g.world._lookPitch=0;const dir=facing==='rear'?1:-1;g.player.faceDir(0,dir);g.player.aim3.set(0,0,dir);window.guardObservation=[];let count=0;function observe(){const g=PW.game;guardObservation.push({time:g.time,guard:g.player.guarding,incoming:g.ms.threatLab.meleeTrial.records.filter(r=>r.incoming).length});if(guardObservation.length>120)guardObservation.shift();if(++count<600&&!guardObservation.at(-1).incoming)requestAnimationFrame(observe);}requestAnimationFrame(observe);},facing);
 let records=[];
 for(let i=0;i<5&&!records.length;i++){await performAction(page,'guard',{holdMs:1500});records=await page.evaluate(()=>PW.game.ms.threatLab.meleeTrial.records.filter(r=>r.incoming));}
 assert(records.length,'Trainer must make actual contact; invulnerability or a miss is not a successful guard test');
 ...
 await writeFile(out+'/result.json',JSON.stringify({passed:true,facing,observation,hit,errors:diagnostics.entries,staging:'Native character/squad menus; defend trial and initial positions/camera staged once. Shared guard action sends Q through browser input. Trainer uses native attack/contact. No damage, energy, immunity or outcome injection. Silent clip; configured facing against a light strike only, not guard crush or grabs.'},null,2));
