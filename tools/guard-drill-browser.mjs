...
try{
 ...
 await writeFile(out+'/result.json',JSON.stringify({passed:true,facing,attack,scheme,observation,hit,grab,errors:diagnostics.entries,staging:'Native character/squad menus; guard/defend trial and initial positions/camera staged once. Shared combat actions use the configured KBM or browser-emulated gamepad through the real input adapter; menus use keyboard. Trainer uses native attack/contact. No damage, energy, immunity or outcome injection. Silent clip; configured light defense, charged-heavy or grab/release counterplay only; grab escape/counterstrikes and depleted energy remain separate.'},null,2));
