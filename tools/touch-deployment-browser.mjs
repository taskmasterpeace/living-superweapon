...
const out='artifacts/marketing/touch-deployment';await mkdir(out,{recursive:true});
...
try{
 await page.goto('http://127.0.0.1:5182/powerworld.html');await page.locator('#hSelect.on').waitFor();await page.waitForTimeout(1200);await page.getByRole('button',{name:'Next character',exact:true}).tap();await page.screenshot({path:out+'/selector.png'});await page.getByRole('button',{name:'Select character',exact:true}).tap();await page.getByRole('button',{name:'Enter with squad',exact:true}).tap();
 ...
 await page.getByRole('button',{name:'READY',exact:true}).tap();
 ...
 await page.waitForFunction(()=>window.PW.game.ms.threatLab.state==='field',{}, {timeout:10000});
 await page.screenshot({path:out+'/deployed.png'});
 ...
 await page.screenshot({path:out+'/returned.png'});await writeFile(out+'/result.json',JSON.stringify({kind:'Touch-only selection, inventory, second gadget, READY tap and touch-stick portal crossing',result,errors},null,2));if(errors.length)throw Error(errors.join('\n'));
