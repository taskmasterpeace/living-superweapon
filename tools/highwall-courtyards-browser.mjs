...
const out='artifacts/highwall/courtyards-native';await mkdir(out,{recursive:true});
...
try{
 ...
 report.distance=await page.evaluate(()=>{const g=PW.game,p=g.player,e=g._highwall.units[1];p.pos.set(212,130,-200);e.pos.set(212,130,700);p.aim3.set(0,0,1);p.aim.set(0,0,1);g.updateVision(.1);return {distance:p.pos.distanceTo(e.pos),visible:e.obj.visible,range:String(g.characterSightRange)};});
