...
for(const id of ['bat','tomahawk','katana','bat']){
 ...
 for(let i=0;i<65;i++){
  const sample=await page.evaluate(i=>{
   ...
   if(i===15)TYPES.melee(f,st.def,st,g,{pressed:true,dt:1/60});
  ...
  if([10,23,35,60].includes(i)){await page.screenshot({path:`${out}/${id}-${i}.png`});await page.evaluate(s=>review.results.push(s),sample);}
