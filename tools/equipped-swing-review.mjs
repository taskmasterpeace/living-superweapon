...
const view=args.find(a=>a.startsWith('--view='))?.split('=')[1]||'overview';
const views={overview:[10,8,12],front:[0,6,9],left:[-9,6,0],right:[9,6,0],rear:[0,6,-9]};
if(!views[view])throw new Error('Unknown view: '+view);
const ids=args.filter(a=>!a.startsWith('--')),out=args.find(a=>a.startsWith('--out='))?.slice(6)||(guard?`artifacts/marketing/weapon-guard-${hero}`:ids.length?`artifacts/marketing/${ids.includes('claws')?'claws':'great-blade'}-review`:'artifacts/marketing/equipped-swing-review');await mkdir(out,{recursive:true});
...
for(const id of ids.length?ids:['bat','tomahawk','katana','bat']){
 ...
 for(let i=0;i<(guard?150:id==='claws'?125:65);i++){
  const sample=await page.evaluate(({i,guard,cameraOffset,view})=>{
   ...
   const [x,y,z]=cameraOffset;v.camera.position.copy(f.pos).add({x,y,z});v.camera.lookAt(f.pos.x,f.pos.y+5,f.pos.z);v.renderer.render(v.scene,v.camera);
   return {weapon:f._gearMesh.userData.weaponKind,elapsed:f._abilityMeleePose?.elapsed,view};
  },{i,guard,cameraOffset:views[view],view});
