...
const out='artifacts/marketing/squad-effects-audit';await mkdir(out,{recursive:true});
...
try{
 ...
 await page.waitForTimeout(5000);const trace=[];let index=1,captured=false;
 ...
 for(let tick=0;tick<360;tick++){
  ...
  if(!captured&&index>=8){captured=true;await page.screenshot({path:out+'/contact.png'});const meshes=await page.evaluate(()=>{const g=window.PW.game,list=[];g.scene.traverse(o=>{if(!o.isMesh||!o.visible||!o.material?.transparent)return;const p=o.getWorldPosition(o.position.clone()),s=o.getWorldScale(o.scale.clone());if(p.distanceTo(g.player.pos)>300)return;o.geometry?.computeBoundingSphere();const radius=(o.geometry?.boundingSphere?.radius||0)*Math.max(s.x,s.y,s.z);if(radius>20)list.push({name:o.name,type:o.geometry?.type,radius,position:p.toArray(),opacity:o.material.opacity,color:o.material.color?.getHexString(),blending:o.material.blending,parent:o.parent?.name});});return list;});await writeFile(out+'/meshes.json',JSON.stringify(meshes,null,2));}if(state.finished)break;
 ...
 const result=await page.evaluate(()=>{const g=window.PW.game,o=g.ms.convoyOperation;return {state:o.state,events:o.events,squad:g.ms.squad.members.map(f=>({name:f.def.name,hp:f.hp,kills:f.kills,x:f.pos.x,z:f.pos.z})),playerHp:g.player.hp,scientistHp:o.scientist.hp,save:g.campaign.snapshot()};});
