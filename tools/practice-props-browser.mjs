...
try{
 ...
 await p.keyboard.press('e');await p.waitForFunction(()=>!window.PW.game.player._carry&&!window.PW.game._flung?.length,{},{timeout:10000});
 ...
 await writeFile(out+'/result.json',JSON.stringify({setup:'Controlled placement beside rock and reset station; native E pickup, throw and reset; reset denial checked through owner while carrying',...result,errors},null,2));console.log(result,errors);if(!result.sameRecord||!result.countStable||errors.length)throw Error('Practice props failed');
