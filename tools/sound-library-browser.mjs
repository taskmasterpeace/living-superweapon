...
try{
 ...
 await page.getByRole('button',{name:'Play cue',exact:true}).click();
 await page.waitForFunction(()=>STUDIO.soundLibrary.lastPlayback?.source==='synthesized-placeholder');
 ...
 await page.getByRole('button',{name:'Play cue',exact:true}).click();await page.waitForFunction(()=>slPeak>.0001);
 ...
 await page.getByRole('button',{name:'Play cue',exact:true}).click();await page.waitForFunction(()=>[...STUDIO.soundLibrary.active].some(h=>h.loop));
 ...
 await page.getByRole('button',{name:'Play cue',exact:true}).click();await page.waitForFunction(()=>STUDIO.soundLibrary.lastPlayback.source==='chosen-recording');
 ...
 await page.getByRole('button',{name:'Sound Library',exact:true}).click();await page.getByRole('button',{name:'Play cue',exact:true}).click();
