...
try{
 await page.goto(`${base}/sound-library.html`);await page.waitForFunction(()=>!!window.AUDIO_WORKSHOP?.library,{},{polling:100});
 assert.equal(await page.evaluate(()=>AUDIO_WORKSHOP.backend.ctx),null);
 ...
 await page.waitForFunction(()=>AUDIO_WORKSHOP.library.state.bindings.light);
 await page.getByRole('button',{name:'Play recording',exact:true}).click();
 await page.waitForFunction(()=>AUDIO_WORKSHOP.library.lastPlayback?.source==='chosen-recording');
 await page.evaluate(()=>{const a=AUDIO_WORKSHOP.backend;window.slProbe=a.ctx.createAnalyser();slProbe.fftSize=2048;a.master.connect(slProbe);window.slPeak=0;window.slTimer=setInterval(()=>{const b=new Float32Array(2048);slProbe.getFloatTimeDomainData(b);for(const n of b)slPeak=Math.max(slPeak,Math.abs(n));},10);const dest=a.ctx.createMediaStreamDestination();a.master.connect(dest);window.slChunks=[];window.slRecorder=new MediaRecorder(dest.stream);slRecorder.ondataavailable=e=>slChunks.push(e.data);slRecorder.start();});
 await page.getByRole('button',{name:'Play recording',exact:true}).click();await page.waitForFunction(()=>slPeak>.0001);
 result.oneShot=await page.evaluate(()=>({playback:AUDIO_WORKSHOP.library.lastPlayback,peak:slPeak}));
 await page.getByLabel('Search sound cues',{exact:true}).fill('hover');await page.locator('[data-cue="hover"]').click();
 await page.getByLabel('Existing sample',{exact:true}).selectOption('spaceEngineLow_000');await page.waitForFunction(()=>AUDIO_WORKSHOP.library.state.bindings.hover);
 await page.getByRole('button',{name:'Play recording',exact:true}).click();await page.waitForFunction(()=>[...AUDIO_WORKSHOP.library.active].some(h=>h.loop));
 result.loop=await page.evaluate(()=>AUDIO_WORKSHOP.library.lastPlayback);await page.getByRole('button',{name:'Stop cue',exact:true}).click();
 assert.equal(await page.evaluate(()=>AUDIO_WORKSHOP.library.active.size),0);
 await page.getByLabel('Search sound cues',{exact:true}).fill('light');await page.locator('[data-cue="light"]').click();
 await page.getByLabel('Choose local sound recording',{exact:true}).setInputFiles({name:'contact-proof.wav',mimeType:'audio/wav',buffer:wav()});
 await page.waitForFunction(()=>AUDIO_WORKSHOP.library.state.bindings.light?.name==='contact-proof.wav');
 await page.getByRole('button',{name:'Play recording',exact:true}).click();await page.waitForFunction(()=>AUDIO_WORKSHOP.library.lastPlayback.source==='chosen-recording');
 ...
 await page.getByRole('status').filter({hasText:'Cannot decode'}).waitFor();assert.equal(await page.evaluate(()=>AUDIO_WORKSHOP.library.state.bindings.light.name),'contact-proof.wav');
 ...
 await page.reload();await page.waitForFunction(()=>!!window.AUDIO_WORKSHOP?.library,{},{polling:100});assert.equal(await page.evaluate(()=>AUDIO_WORKSHOP.library.state.bindings.light.name),'contact-proof.wav');
 await page.getByRole('button',{name:'Play recording',exact:true}).click();
 ...
 result.transfer=await page.evaluate(()=>({source:AUDIO_WORKSHOP.library.source('light'),decoded:AUDIO_WORKSHOP.library.buffers.has('light')}));
