...
test('review interpolates real rig poses without altering native fighter or shared materials',()=>{
 const x=mainCombatFixture({hero:'rage'});try{
  ...
  x.p.obj.position.x=10;x.p.parts.armR.rotation.x=.8;x.p.hp=23;r.capture(.1);
...
test('history is bounded and transient effect nodes cannot erase the exchange',()=>{
 ...
 obj.add(new THREE.Object3D());r.capture(10);assert.ok(r.frames.length>1);assert.equal(r.frames.at(-1).actors[0].pose.length,11);assert.equal(r.actors[0].template.nodes.length,1);
