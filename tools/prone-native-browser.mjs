...
const out=process.argv[2]||'artifacts/prone-native';await mkdir(out,{recursive:true});
...
const result={scope:'Native SARGE Practice, Z prone, W crawl with LMB fire, R reload, Z stand. No pose/actor/camera/resource edits.',viewport,errors:[],phases:[]};page.on('pageerror',e=>result.errors.push(String(e)));
...
try{
 ...
 await page.waitForFunction(()=>PW.game.pwStage?.frontlineReady&&!PW.game._frontlinePreparing,null,{timeout:90000});await page.bringToFront();await page.mouse.click(viewport.width/2,viewport.height/2);await page.waitForFunction(()=>!!document.pointerLockElement);await page.waitForTimeout(500);
