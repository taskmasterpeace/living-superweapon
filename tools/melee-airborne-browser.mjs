...
const out='artifacts/marketing/airborne-trial-2026-09-12';await mkdir(out,{recursive:true});
...
try{
 ...
 await page.evaluate(()=>{const g=window.PW.game,t=g.ms.threatLab.meleeTrial;t.start('airborne');g.player.pos.copy(t.origin).add({x:0,y:0,z:-10});g.player.vel.set(0,0,0);g.world._lookYaw=0;g.world._lookPitch=0;});
 ...
 await page.keyboard.press('v');await page.waitForTimeout(1800);
 ...
 const contact=page.getByRole('button',{name:/s · CONTACT$/});if(await contact.count())await contact.first().click();
 ...
 report.errors=errors;await writeFile(out+'/result.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
