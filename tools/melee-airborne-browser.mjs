...
const defense=process.argv.includes('--air-defense'),catchFall=process.argv.includes('--catch'),friendly=catchFall||process.argv.includes('--friendly'),grab=friendly||process.argv.includes('--grab');const out=`artifacts/marketing/${defense?'air-defense':catchFall?'friendly-catch':friendly?'friendly-carry':grab?'airborne-throw':'airborne-trial'}-2026-09-12`;await mkdir(out,{recursive:true});
...
try{
 ...
 await page.evaluate(({grab,defense})=>{const g=window.PW.game,t=g.ms.threatLab.meleeTrial;t.start(defense?'air-defense':'airborne');g.player.pos.copy(t.origin).add({x:0,y:0,z:grab?-5:-10});g.player.vel.set(0,0,0);g.world._lookYaw=0;g.world._lookPitch=0;},{grab,defense});
 if(defense)await page.keyboard.down('q');
 ...
 }else if(defense){await page.waitForFunction(()=>window.PW.game.ms.threatLab.meleeTrial.records.some(r=>r.incoming&&r.result==='BLOCK'),{},{timeout:15000});await page.keyboard.up('q');}
 ...
 const contact=page.getByRole('button',{name:defense?/s · BLOCK$/:grab?/s · TERRAIN IMPACT$/:/s · CONTACT$/});if(await contact.count())await contact.first().click();
 ...
 if(defense)report.setup='Controlled ground placement; native Space takeoff and held Q against an airborne native attacker';
 ...
 if(errors.length||(defense?!report.records.some(r=>r.incoming&&r.result==='BLOCK'&&r.healthLost===0):friendly?report.target.hp!==130:!report.records.some(r=>r.healthLost>0)))throw Error('Aerial native outcome not proven');
