...
try{
 ...
 if(process.argv.includes('--cover')){
  await page.keyboard.down('d');await page.waitForTimeout(1900);await page.keyboard.up('d');await page.waitForTimeout(300);
  const target=await page.evaluate(()=>{const g=PW.game,p=g.player,w=g.world;const c=w.cover.filter(c=>c.hp>0&&c.top>5&&!c.vehicle&&Math.hypot(c.x-p.pos.x,c.z-p.pos.z)<125).sort((a,b)=>Math.hypot(a.x-p.pos.x,a.z-p.pos.z)-Math.hypot(b.x-p.pos.x,b.z-p.pos.z))[0];if(!c)throw Error('No existing cover within range');window.__coverHits=[];const previousHp=new Map(w.cover.map(c=>[c,c.hp])),cracks=w.setBlockCracks;w.setBlockCracks=function(c,...rest){const before=previousHp.get(c);window.__coverHits.push({time:g.time,before,after:c.hp,x:c.x,z:c.z,kind:c.frontlineBuilding||null});previousHp.set(c,c.hp);return cracks.call(this,c,...rest);};const original=g.damageBlock;g.damageBlock=function(c,amount,pos,...rest){const before=c.hp,result=original.call(this,c,amount,pos,...rest);window.__coverHits.push({time:g.time,before,after:c.hp,amount,pos:pos?.toArray(),x:c.x,z:c.z});return result;};const dx=c.x-w.camera.position.x,dz=c.z-w.camera.position.z;return {x:c.x,z:c.z,hp:c.hp,yaw:Math.atan2(dx,dz),pitch:Math.atan2(Math.min(c.top-1,8)-w.camera.position.y,Math.hypot(dx,dz)),lookYaw:w._lookYaw,lookPitch:w._lookPitch,sens:w._lookSens};});
  result.coverTarget=target;await page.mouse.move(720-Math.atan2(Math.sin(target.yaw-target.lookYaw),Math.cos(target.yaw-target.lookYaw))/target.sens,450+(target.lookPitch-target.pitch)/target.sens);await page.waitForTimeout(250);
 }
 ...
 for(const hold of (cleanup?[450,1150,450,1150]:[450,1150])){
  ...
  await page.waitForTimeout(process.argv.includes('--cover')?700:120);const beam=await page.evaluate(()=>{const b=PW.game.player.slots.rmb.active;return {radius:b.radius,dps:b.dps,range:b.maxLen,tip:b.tip.position.toArray(),muzzle:b.muzzle.toArray(),blocked:b.blocked,contactKind:b._obstacleContact?.kind,contactTarget:b._obstacleContact?.target&&{x:b._obstacleContact.target.x,z:b._obstacleContact.target.z,hp:b._obstacleContact.target.hp},groundY:PW.game.world.heightAt(b.tip.position.x,b.tip.position.z)};});await page.screenshot({path:out+'/travel-'+hold+'.png'});
 ...
 assert.ok(result.shots[1].held.charge>result.shots[0].held.charge);assert.ok(result.shots[1].beam.radius>result.shots[0].beam.radius);assert.equal(result.shots[1].blasts.length,0,'Violet Lance does not enable remote detonation');assert.deepEqual(result.errors,[]);if(process.argv.includes('--cover')){result.coverHits=await page.evaluate(()=>window.__coverHits);assert.ok(result.coverHits.some(h=>h.after<h.before),'Native beam must damage existing cover');}result.passed=true;
