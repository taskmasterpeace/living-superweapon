...
try{
 await page.goto((process.env.LSW_TEST_URL||'http://127.0.0.1:5184')+'/studio.html?hero=vega');await page.waitForFunction(()=>window.STUDIO?.soundLibrary);
 ...
 await page.waitForFunction(()=>STUDIO.soundLibrary.cues.some(c=>c.label==='Research lab ventilation'));
 await page.getByLabel('Existing sample',{exact:true}).selectOption('spaceEngineLow_000');
 await page.waitForFunction(()=>Object.values(STUDIO.soundLibrary.state.bindings).some(b=>b.name==='spaceEngineLow_000.mp3'));
 await page.getByRole('button',{name:'Play recording',exact:true}).click();await page.waitForFunction(()=>STUDIO.soundLibrary.lastPlayback?.loop);await page.getByRole('button',{name:'Stop cue',exact:true}).click();
 ...
 const id=await page.evaluate(()=>STUDIO.soundLibrary.cues.find(c=>c.label==='Research lab ventilation').id);
 await page.getByLabel('Reuse recording',{exact:true}).selectOption(id);await page.waitForFunction(()=>STUDIO.soundLibrary.state.bindings['weather-rain']?.name==='spaceEngineLow_000.mp3');
 await page.reload();await page.waitForFunction(()=>window.STUDIO?.soundLibrary);await page.getByRole('button',{name:'Sound Library',exact:true}).click();await page.getByLabel('Sound type',{exact:true}).selectOption('loop');await page.getByLabel('Recording status',{exact:true}).selectOption('assigned');
