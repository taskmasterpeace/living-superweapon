...
const out='artifacts/marketing/threat-deployment-five';await mkdir(out,{recursive:true});
...
try{
 ...
 await dialog.getByRole('button',{name:'LSW SIDE',exact:true}).click();
 for(let i=0;i<5;i++)await dialog.locator('.squad-roster button').nth(i).click();
 const selected=await dialog.locator('.squad-roster button[aria-pressed=true]').allInnerTexts();
