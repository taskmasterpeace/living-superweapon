...
const {defense=false,throwTarget=false}=JSON.parse(process.env.PW_PLAYTEST_CONFIG||'{}');
...
try{
 ...
 await p.mouse.move(640,throwTarget?180:360);
 await stageMelee(p,{trial:defense?'air-defense':'airborne',distance:throwTarget?6:30});
 ...
 if(throwTarget){
 for(let i=0;i<12;i++){
 const gap=await p.evaluate(()=>PW.game.player.pos.y-PW.game.ms.threatLab.meleeTrial.target.pos.y);if(Math.abs(gap)<2)break;
 await performAction(p,gap>0?'down':'up',{holdMs:80});await p.waitForFunction(()=>Math.abs(PW.game.player.vel.y)<2,null,{timeout:5000});
 }
 }
 for(let i=0;i<2;i++){
 ...
 assert(before.playerAir&&before.targetAir);if(!defense)assert.equal(before.locked,before.target);if(!defense&&!throwTarget)assert(before.distance>15,'Must begin outside ordinary fist reach');
 let block=null,throwResult=null;
 if(throwTarget){
 await performAction(p,'grab');await p.waitForFunction(()=>!!PW.game.player._personCarry,null,{timeout:5000});
 await saveSnapshot(p,out,{name:'air-carry'});await p.screenshot({path:out+'/air-carry.png'});
 const look=await p.evaluate(()=>({x:PW.game.input._lastCX,y:PW.game.input._lastCY,pitch:PW.game.world._lookPitch,sens:PW.game.world._lookSens}));
 await p.mouse.move(look.x,look.y-(-.9-look.pitch)/look.sens);await p.waitForTimeout(150);
 await performAction(p,'grab',{until:'grab-armed'});
 await p.waitForFunction(()=>PW.game.ms.threatLab.meleeTrial.records.some(r=>r.result==='TERRAIN IMPACT'&&r.healthLost>0),null,{timeout:15000});
 throwResult=await p.evaluate(()=>{const g=PW.game,t=g.ms.threatLab.meleeTrial;return {records:t.records,held:!!g.player.grabbing,carry:!!g.player._personCarry,victimHeld:!!t.target.grabbedBy};});
 assert(!throwResult.held&&!throwResult.carry&&!throwResult.victimHeld);assert(throwResult.records.some(r=>r.move==='throw'&&r.healthLost>0));
 }else if(defense){
 ...
 await p.screenshot({path:out+'/air-contact.png'});await saveSnapshot(p,out,{name:'observed-state',errors:diagnostics.entries});await writeFile(out+'/result.json',JSON.stringify({passed:true,defense,throwTarget,throwResult,block,before,after,actions:actionHistory(p),errors:diagnostics.entries,staging:'One ground setup at 30u for approach/guard or 6u for throw. Native held Space takes off, trainer natively rises and hovers, Mouse aims; approach mode uses T lock and V strike, defense mode faces the attacker and holds Q without requiring lock; throw mode aligns height with bounded native rise/descend pulses, grabs with E, aims down with the mouse and holds/releases E after native throw arming. No altitude, flight flag, motion or damage injection after setup. This tests the configured approach, guard or throw sequence, not evasion or full air-combat balance.'},null,2));
