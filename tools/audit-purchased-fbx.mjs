...
for(const pack of manifest.packs)for(const file of pack.files||[]){
 ...
 try{
  ...
  results.push({pack:pack.id,path:file.path,bones: bones.length,boneNames:bones,clips,skippedTextures:[...new Set(skippedTextures)],status:clips.length&&clips.every(c=>c.valid&&c.finite&&c.duration>0&&c.tracks>0)?'source-parsed':'needs-review'});
...
await fs.writeFile(path.join(root,'fbx-audit.json'),JSON.stringify({acceptance:'Source parse and finite tracks only; no retarget, visual or gameplay approval',summary,results},null,2));
