...
test('a cover exit commits a jointly compatible triangle rather than just its feasible corners',()=>{
 const f=new Fighter(ROSTER.find(d=>d.id==='sol'));try{
  f._animate(0);const cloth=new RagdollCape(f.parts,new T.Vector3()),face=[40,41,53].map(i=>cloth.points[i]);
  face[0].pos.set(2.1,5,-2);face[1].pos.set(2.1,6,2);face[2].pos.set(2.1,7,2);
  for(const p of face){p.prev.copy(p.pos);p.last.copy(p.pos);p.faces=[face];}
  cloth.recordSurfaceContact(face,[.5,.5,0],new T.Vector3(1,0,0),2);
  const box=new T.Box3(new T.Vector3(2,-10000,-1),new T.Vector3(3,20,1));
  cloth.faces=[face];cloth.covers=[{cover:true,box,worldBox:box.clone(),matrix:new T.Matrix4(),inverse:new T.Matrix4()}];
  cloth.collideCoverSurface({heightAt:()=>0});
  assert.ok(!box.intersectsTriangle(new T.Triangle(...face.map(p=>p.pos))),'partially committed cover exit left the triangle intersecting cover');
  assert.ok((face[0].pos.x+face[1].pos.x)/2>=2-1e-9,'cover exit invalidated the earlier body sample');
 }finally{f.dispose();}
});
