...
export async function performAction(page,name,{holdMs=80,until=null,scheme='kbm',move=null}={}){
 ...
 if(move!==null&&(scheme!=='pad'||!Array.isArray(move)||move.length!==2||!move.every(v=>Number.isFinite(v)&&v>=-1&&v<=1)))throw new Error('Movement requires pad scheme and two axes from -1 to 1');
 ...
 try{
  ...
  record.simulationBefore=before.time;if(move){moving=true;await gamepadMove(page,...move);}await down();sent=true;if(until==='melee-charged'){await page.waitForFunction(()=>globalThis.PW?.game?.player?.meleeCharge>=.6,null,{timeout:10000,polling:100});record.chargeAtRelease=await page.evaluate(()=>PW.game.player.meleeCharge);}else await page.waitForTimeout(holdMs);
  await up();sent=false;record.released=true;if(moving){await gamepadMove(page,0,0);moving=false;record.movementReleased=true;}
  ...
  finally{try{if(moving){await gamepadMove(page,0,0);record.movementReleased=true;}}finally{active.delete(page);}}
 }
 return {...record};
}
