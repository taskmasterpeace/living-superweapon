import {sessionSummary} from './session.mjs';
...
export async function saveSnapshot(page,out,{error=null,errors=[],name='failure'}={}){
 const bundle={version:1,session:sessionSummary(page),actions:actionHistory(page),capturedAt:new Date().toISOString(),error:error?{name:error.name,message:String(error.message).slice(0,4000)}:null,errors:errors.slice(-40)};
