...
const out='artifacts/marketing/reserve-replacement';await mkdir(out,{recursive:true});
...
try{
 await page.goto('http://127.0.0.1:5182/powerworld.html');await page.locator('#hSelect.on').waitFor();await page.waitForTimeout(1000);await page.keyboard.press('Enter');await page.locator('.squad-roster button').first().click();await page.getByRole('button',{name:'Enter with squad',exact:true}).click();
 await page.waitForFunction(()=>window.PW?.game?.ms?.threatLab?.state==='preparing',{}, {timeout:90000});await page.waitForTimeout(1200);await page.keyboard.press('g');await page.keyboard.down('w');await page.waitForFunction(()=>window.PW.game.ms.threatLab.deployed.has(window.PW.game.player),{}, {timeout:8000});await page.keyboard.up('w');await page.waitForFunction(()=>window.PW.game.ms.threatLab.state==='field');
 ...
 await page.waitForTimeout(500);await page.screenshot({path:out+'/request.png'});await page.keyboard.press('g');
 await page.waitForFunction(()=>window.PW.game.ms.squad.members[0]!==window.lostCompanion,{}, {timeout:5000});
 for(let i=0;i<3;i++){await page.keyboard.press('g');await page.waitForTimeout(150);}
 ...
 await page.screenshot({path:out+'/replaced.png'});await writeFile(out+'/result.json',JSON.stringify({kind:'Native menu/deployment; injected lethal damage to companion and staged player at reserve point; native G replacement and repeat presses',result,errors},null,2));
 if(result.after.remaining.lsw!==result.before.remaining.lsw-1||result.members.length!==1||!result.members[0].alive||!result.sameLeader||errors.length)throw Error('Reserve replacement failed');
