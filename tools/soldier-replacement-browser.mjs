...
const out='artifacts/marketing/soldier-replacement';await mkdir(out,{recursive:true});
...
try{
 await page.goto('http://127.0.0.1:5182/powerworld.html');await page.locator('#hSelect.on').waitFor();await page.waitForTimeout(1000);await page.locator('.scard').filter({has:page.locator('.snm',{hasText:/^SARGE$/})}).click();await page.keyboard.press('Enter');await page.getByRole('button',{name:'SOLDIER SIDE',exact:true}).click();await page.getByRole('button',{name:'+ Soldier',exact:true}).click();await page.getByRole('button',{name:'Enter with squad',exact:true}).click();
