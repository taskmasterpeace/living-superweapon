...
export class HighwallNavigation {
 ...
 replace(solids,revision){this.solids=solids.map(s=>({...s}));this.revision=revision;this.cache=new Map();}
 ...
 _boxes(shape){return this.solids.filter(s=>(s.bottom??0)<=shape.height&&(s.top??Infinity)>0);}
 isClear(p,options){
  ...
  return !this._boxes(s).some(o=>Math.abs(p.x-o.x)<=o.hx+r&&Math.abs(p.z-o.z)<=o.hz+r);
 }
