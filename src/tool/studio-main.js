...
function mutate(path,value,rebuild=false){const p=history.value;let part=p;const keys=path.split('.');for(const k of keys.slice(0,-1))part=part[k];part[keys.at(-1)]=value;try{applyProfile(hero,p);history.push(p);preview.setProfile(hero,p,rebuild);savedState();status('Draft updated. Save local to use it in the next game.');}catch(e){status(e.message,true);inspector();}}
