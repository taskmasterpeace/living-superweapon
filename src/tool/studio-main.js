...
import {GROUND_APPROACH_FAMILIES,meleeApproach} from '../data/melee-approaches.js';
...
<main class="workspace"><aside class="library"><div class="section-title">Fighters <span class="count">${ROSTER.length}</span></div><input class="search" aria-label="Find fighter" placeholder="Find a fighter…"><div class="roster"></div><div class="library-footer">Studio profiles include presentation and sparse attack tuning.<br>Local saves never edit source files.<br><a href="./animation-library.html">Animation Library ↗</a><br><a href="./index.html">Power kits / ORIGIN ↗</a></div></aside>
...
$('.library-footer a:last-of-type').href='./index.html?editor=origin';
$('.actions').insertAdjacentHTML('afterbegin','<a href="./index.html" style="color:#dfc170;padding:10px">← Deployment hub</a>');
$('.library-footer').insertAdjacentHTML('beforeend','<br><a href="./asset-library.html?collection=characters">Construct & creature library ↗</a>');
...
$('.combat-controls').insertAdjacentHTML('afterend',`<div class="combat-controls melee-controls" hidden><label>Sequence <select id="melee-sequence" aria-label="Melee sequence">${Object.entries(MELEE_SEQUENCES).map(([id,name])=>`<option value="${id}">${name}</option>`).join('')}</select></label><p id="melee-style"></p><p>PowerWorld controls: hold Q to guard · tap V: melee · hold V: heavy · E: interact/grab. Hold E and release to throw; tap E to set down. Double-tap a direction to dodge. Approach rehearsal starts at 65% of this character’s native entry range; gameplay decides movement and contact.</p><p>Production contact and physics vs KANO · silent rehearsal · resistance applies. Funded frontal guard protects health by spending energy; grabs and heavy guard crush counter it. Custom fighters inherit weight from Might, guard type and strike access from Edit power kit.</p></div>`);
...
$('.melee-controls p:last-child').textContent='Production contact and physics vs KANO (SOL for guard-breaking heavy) · optional sound, not an AI balance test. Funded frontal guard spends energy before health; grabs and heavy guard crush counter it. Custom fighters inherit Might, guard type and Melee Style from Edit power kit.';
...
function selectHero(def){let p,dropped=[],saved=false;try{p=loadProfile(def.id);saved=!!p||!!def.isCustom;p||=profileFromDef(def);({profile:p,dropped}=reconciledProfile(def,p));applyProfile(def,p);status(dropped.length?`${def.name} loaded. Dropped stale attack tuning for ${dropped.map(slot=>slot.toUpperCase()).join(', ')} because the ORIGIN attack changed. Saved storage was not overwritten.`:`${def.name} loaded. Saved profiles apply when a game page loads; source files stay unchanged.`);}catch(e){p=profileFromDef(def);applyProfile(def,p);status(e.message,true);}if(new URLSearchParams(location.search).get('body')==='faceted-v1'){p.model.body='faceted-v1';saved=false;}preview.setProfile(def,p);hero=def;hasSaved=saved;const slots=new Set([...Object.keys(def.abilities||{}),...kitAlternativeSlots(def.id)]);if(!slots.has(attackSlot))attackSlot=Object.keys(def.abilities||{})[0]||kitAlternativeSlots(def.id)[0]||'';history=new DraftHistory(p);syncCombat();$('.hero-heading').innerHTML=`${esc(hero.name)}<small>${esc(hero.title)} / ${esc(hero.id)}</small>`;roster();inspector();savedState();}
...
const flightLabels={...CRUISE_STYLE_LABELS,hero:'One-fist lead',twin:'Two-fist spearhead',martial:'BFP arms-back',thruster:'Repulsor stance',hammer:'Weapon-led flight',glider:'Relaxed glide'};
...
function inspector(){const p=history.value;let html='';
 ...
 if(tab==='model')html+=`<h3>Fighting motion</h3>${menu('combat.groundApproach','Melee approach',GROUND_APPROACH_FAMILIES,p.combat?.groundApproach??'auto')}<p class="help" id="approach-summary">${meleeApproach(applyProfile(hero,p)).family.toUpperCase()} · ${meleeApproach(applyProfile(hero,p)).range}u approach range · ${meleeApproach(applyProfile(hero,p)).speed}u/s entry cap. Auto uses this character’s default. Shared entry into your strike; flight capability is unchanged. Test contact and counterplay in the Threat Room.</p>${menu('model.strikes','Light strike animation',['authored','procedural'],p.model.strikes??'authored')}${menu('model.heavyStrikes','Heavy punch animation',['authored','procedural'],p.model.heavyStrikes??'authored')}<p class="help">Authored jab, cross and heavy hook use licensed CC0 motion, retimed to this fighter’s combat windows. Ground strikes plant a fighting stance; airborne strikes retain superhero legs. The hook includes its recovery. Armed heavy swings and two-person grabs keep their procedural motion. Choose Melee sequence, then Grounded or Airborne beneath the stage.</p>`;
...
$('.inspector-body').addEventListener('change',async e=>{const el=e.target;
 ...
 const isNumber=el.type==='range'||el.type==='number';if(isNumber&&!el.checkValidity()){status('Enter a value inside the displayed range.',true);return;}mutate(el.dataset.path,isNumber?Number(el.value)/(+el.dataset.factor||1):el.value,/^(frame|colors|model|effects|combat)\./.test(el.dataset.path));if(isNumber)el.closest('.property-control').querySelectorAll('input').forEach(input=>input.value=el.value);
 if(el.dataset.path==='combat.groundApproach'){inspector();$('[data-path="combat.groundApproach"]').focus();}
...
$('.inspector-body').addEventListener('click',e=>{
 ...
 if(button.id==='preview-shield'){
  preview.combat.meleeSequence='block';preview.combat.meleeStage='grounded';$('#melee-sequence').value='block';$('#melee-stage').value='grounded';setState('melee');setView('orbit');status('Incoming punch uses real energy-first guard and localized shield contact.');return;
 }
