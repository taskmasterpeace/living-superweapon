...
function applyRecipe(sync=false){
 ...
 if(sync){size=recipe.size??1;if(![...document.querySelector('#size').options].some(o=>+o.value===size))document.querySelector('#size').add(new Option('Custom height '+size,String(size)));document.querySelector('#size').value=String(size);applyModularFrame(actor,recipe.frame,height/1.8325*size);
  ...
  for(const key of ['capeStyle','collarStyle','hoodie','handModule','auraOrigin','glowRegion','aura','auraState','clawStyle','tattooRegion','headwear','wings','coatStyle','patternRegion','shieldStyle','patternScale','infection','pattern','footwear','beltStyle','hair','frame','emblem','emblemPlacement','expression','muscle','anatomy','gloves'])document.getElementById(key).value=recipe[key]??(key==='collarStyle'?'classic':key==='auraOrigin'?'body':['hoodie','handModule','glowRegion'].includes(key)?'none':key==='aura'?'none':key==='auraState'?'always':key==='clawStyle'?'paired':key==='tattooRegion'?'upperArmL':key==='capeStyle'?'full':['headwear','wings'].includes(key)?'none':key==='coatStyle'?'long':key==='patternRegion'?'all':key==='shieldStyle'?'round':key==='expression'?'neutral':key==='anatomy'?(recipe.frame==='agile'?'female':'male'):1);
...
for(const key of ['capeStyle','collarStyle','hoodie','handModule','auraOrigin','glowRegion','tattooRegion','aura','auraState','clawStyle','headwear','wings','coatStyle','patternRegion','shieldStyle','infection','pattern','footwear','beltStyle','hair','frame','emblem','emblemPlacement','expression','anatomy','gloves'])document.getElementById(key).onchange=e=>{recipe[key]=e.target.value;if(key==='capeStyle'){recipe.cape=true;document.querySelector('#cape').checked=true;}if(key==='collarStyle'){recipe.collar=true;document.querySelector('#collar').checked=true;}applyRecipe();draw();};
