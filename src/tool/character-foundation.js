...
function draw(){
 ...
 signatureParts.set(recipe);signatureParts.update(performance.now()/1000,flight,/Attack|Punch/.test(motion)?'attack':'idle');
...
for(const key of ['aura','auraState','clawStyle','headwear','wings','coatStyle','patternRegion','shieldStyle','infection','pattern','footwear','beltStyle','hair','frame','emblem','emblemPlacement','expression','anatomy','gloves'])document.getElementById(key).onchange=e=>{recipe[key]=e.target.value;applyRecipe();draw();};
document.querySelector('#muscle').oninput=e=>{recipe.muscle=+e.target.value;applyRecipe();draw();};
for(const key of ['skirt','beard','claws','tentacles','wristBlasters','lasso','kilt','metallic','shimmer','coat','wristbands','tornClothes','robe','sleeves','collar','glasses','cape','armor','visor','eyepatch','eyeGlow','gauntlets','shoulders','knees','belt','backpack'])document.getElementById(key).onchange=e=>{recipe[key]=e.target.checked;applyRecipe();draw();};
