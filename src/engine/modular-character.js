...
export async function loadModularCharacter(f,{load=modularAsset}={}){
 ...
 c.update=()=>{
  ...
  const interaction=!!(contactStrike||f._personThrowPose||f._zombieLimbs||f._grapple||f.hanging||f._carry||f.grabState||f.meleeCharge>0||f.crouching||f._jumpMotion?.applied||f.downedT>0||f.launchT>0||f._slideT>0);
