...
export function characterStudyCatalog(actor){
 ...
 return Object.keys(ACTION_DRAFTS).map(name=>{
  ...
  return {clip,entry:{id:'study:'+name,key:name,take:name,label:name,category:'Editable studies',kind:'modular',duration:m.duration,loop:!!m.loop,frames:m.keys.length,
   contact:{start:m.markers.contact,end:m.markers.release},markers:m.markers,hand:m.hand,contactStyle:m.contactStyle,
   source:{author:'Power World',description:m.source},runtime:review?.rejected?'Visual review failed · gameplay assignment blocked':'Editable candidate · gameplay assignment pending',issues:[],warnings:[...(review?[review.note]:[]),'Visual and gameplay contact review required'],audioStatus:'unverified',
