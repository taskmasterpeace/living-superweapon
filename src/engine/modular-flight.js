...
export function createModularFlightAdapter(actor,fighter){
 ...
 for(const [side,arm,leg] of [['L',p.armR,p.legR],['R',p.armL,p.legL]]){
  ...
  const armRegion=arm===p.armL?'armL':'armR';
  add('DEF-upper_arm.'+side,arm.children[0],()=>wp(arm),down,armRegion);
  add('DEF-forearm.'+side,fore,()=>fore.localToWorld(new T.Vector3(0,arm.userData.foreLength/2,0)),down,armRegion);
  add('DEF-hand.'+side,hand,()=>wp(hand),down,armRegion);
