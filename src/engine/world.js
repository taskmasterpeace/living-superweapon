...
export class World {
  ...
  _buildComposer() {
    ...
    this.bloom = new UnrealBloomPass(new THREE.Vector2(innerWidth * 0.5, innerHeight * 0.5), 0.66, 0.7, 0.8);      // half-res bloom (~4× cheaper). Threshold/strength are re-driven per camera in _applyQuality (Refs #42 iter 20).
  ...
  _applyQuality() {
    ...
    // RICHER BLOOM, SCOPED TO THE OPEN-SKY/CHASE PATH (Refs #42 iter 20): Robert's "a lot of bloom"
    // ask, verified on the PowerWorld captures — but the CITY's additive beams are brighter and a
    // lower threshold there risks washout I could not capture to confirm. So the hotter glow rides
    // the chase camera (where the beam game is played) and the iso city keeps its original tuning.
    const rich = this.camMode !== 'iso';
    this.bloom.threshold = rich ? 0.72 : 0.8;
    this.bloom.strength = this._threatRoom?.active ? .12 : rich ? (t === 2 ? 0.72 : t === 1 ? 0.6 : 0.45) : (t === 2 ? 0.66 : t === 1 ? 0.55 : 0.42);
