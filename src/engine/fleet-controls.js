...
export class FleetControlsHud {
 ...
 update(actor,options){
  ...
  const alt=(actor.cls==='fixedwing'||actor.cls==='rotor')?` · ALT ${Math.max(0,Math.round((actor.pos?.y||0)*.19))}m`:'';
  const status=(actor.cls==='fixedwing'?` · THROTTLE ${Math.round((m.lever||0)*100)}% · GEAR ${m.gearDown?'DOWN':'UP'}${m.stalled?' · STALL — NOSE DOWN + THROTTLE':''}`:'')+alt;
  const ammo=actor.weapon?` · ${actor.weapon.ammoLine()}`:'';
  this.title.textContent=`${actor.name||actor.id} · ${Math.round(speed)} km/h${status}${ammo}`;
  this.keys.textContent=fleetControls(actor.cls,{...options,armed:!!actor.weapon});
 }
