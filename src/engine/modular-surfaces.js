...
export function outfitTexture(recipe){
 ...
 if(recipe.pattern==='custom'){
  ...
  t=new T.TextureLoader().load(recipe.patternImage);
   }else if(recipe.pattern==='space'){const c=document.createElement('canvas');c.width=c.height=256;const x=c.getContext('2d');x.fillStyle='#05090f';x.fillRect(0,0,256,256);for(let i=0;i<60;i++){const px=(i*73+17)%256,py=(i*109+41)%256;x.fillStyle=i%4?'#d8e2e9':'#678aaa';const size=i%7?1:3;x.fillRect(px,py,size,size);}t=new T.CanvasTexture(c);
