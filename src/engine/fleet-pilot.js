...
export function fleetIntent(cls, c) {
  if (cls === 'fixedwing') return { throttle: c.throttle, steer: c.bank, rudder: c.bank, pitch: c.pitch, barrel: c.barrel, parked: false };
...
export class FleetPilot {
  constructor(game) { this.game = game; this.actor = null; this._tapT = 0; this._tapKey = ''; this._audio = new FleetAudio(game.audio); }
  ...
  handleInput(input) {
    ...
    let barrel = 0;
    for (const k of ['KeyA', 'KeyD']) if (input?.pressed?.(k)) { const now = (typeof performance !== 'undefined' ? performance.now() : Date.now()); if (this._tapKey === k && now - this._tapT < 300) barrel = k === 'KeyA' ? -1 : 1; this._tapKey = k; this._tapT = now; }
    this._c = {
  ...
  enter(a, p) {
    ...
    this._seat();
    this._audio.enter(a);
    this.game.hud?.feed?.(`${(a.name || a.id).toUpperCase()} — WASD DRIVE · J EXIT`, '#ffce75');
    return true;
  }
