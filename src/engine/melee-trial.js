...
export const MELEE_TRIALS=['stationary','retreat','guard','dodge','defend','airborne'];
...
export class MeleeTrial {
 ...
 start(kind='stationary'){
  ...
  const base=ROSTER.find(d=>d.id===(kind==='airborne'?'sol':'merc'))||ROSTER[0];
 ...
 control(f,dt){
  ...
  if(this.kind==='airborne'){
   ...
   f.flyHeld=altitude<24;f.descendHeld=altitude>28;
