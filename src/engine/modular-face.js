...
export function faceTexture(id='neutral',eyeColor='#171e1b'){
 if(!Object.hasOwn(FACE_EXPRESSIONS,id))throw Error('Unknown facial expression: '+id);
 const cacheKey=id+eyeColor;if(textures.has(cacheKey))return textures.get(cacheKey);
...
export function setModularExpression(meshes,id='neutral',options={}){
 ...
 for(const m of meshes)if(m.userData.slot==='expression'){
  ...
  m.material.map=faceTexture(id,eyeColor);m.material.transparent=true;m.material.depthWrite=false;m.material.alphaTest=.05;m.material.color.set('#ffffff');
