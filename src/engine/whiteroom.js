...
export class WhiteRoom {
  ...
  open() {
    ...
    // floor + calibration grid: distance and knockback have to be readable, not just numbers
    const fl = add(new THREE.Mesh(new THREE.PlaneGeometry(ROOM * 2, ROOM * 2), floor));
    fl.rotation.x = -Math.PI / 2; fl.position.y = 0.06; fl.receiveShadow = true;
    const grid = add(new THREE.GridHelper(ROOM * 2, 26, 0x14120e, 0xa8a49b));
    grid.position.y = 0.09; grid.material.opacity = 0.34; grid.material.transparent = true;
    ...
    for (let i = 0; i < 4; i++) {
      ...
      sk.position.copy(w.position).setY(h - 0.8); sk.rotation.y = a;
    }
    ...
    for (let i = -2; i <= 2; i++) {
      const b = add(new THREE.Mesh(new THREE.BoxGeometry(ROOM * 2, 1.2, 2.4), steel));
      b.position.set(0, CEIL + 3, i * 52); b.castShadow = false;
    }
    for (let i = -1; i <= 1; i++) {                                 // ceiling light strips
      const s = add(new THREE.Mesh(new THREE.BoxGeometry(ROOM * 1.2, 0.5, 1.6), gold));
      s.position.set(0, CEIL + 1.4, i * 52 + 26);
    }
    ...
    for (let i = 0; i < SLEDS; i++) {
      ...
      const rail = add(new THREE.Mesh(new THREE.BoxGeometry(150, 0.6, 0.6), trim));
    ...
    for (let i = 0; i < TURRETS; i++) {
      ...
      const body = add(new THREE.Mesh(new THREE.CylinderGeometry(3, 3.6, 5, 10), steel));
      body.position.set(x, 26, z); body.rotation.z = Math.PI / 2; body.lookAt(0, 26, 0);
      const eye = add(new THREE.Mesh(new THREE.SphereGeometry(1.5, 10, 8), live));
    ...
    for (let i = 0; i < RINGS; i++) {
      ...
      add(r); this.rings.push({ mesh: r, i });
    }
  ...
  cycleDrill() {
    ...
    const next = DRILLS[(i + 1) % DRILLS.length];
  ...
  _drawBoard() {
    ...
    x.textAlign = 'right'; x.fillStyle = GOLD; x.fillText(D.name, 998, 44); x.textAlign = 'left';
    ...
    if (this.drill === 'targets')      { stat(44, 'TARGETS DOWN', String(this.score), GOLD); stat(300, 'TIME', this.drillT.toFixed(0) + 's'); }
    ...
    x.textAlign = 'right'; x.fillText('CONSOLE: CHANGE DRILL   ·   N: DUMMY   ·   SHIFT+R: CLEAR', 998, 122); x.textAlign = 'left';
