...
// Wheel animation (the model ships wheels_FL/FR/RL/RR as identity-framed nodes with
// the axle on local X, radius 0.65 in model units). Preallocated so a frame allocates
// nothing. Front wheels steer about local Y; all four roll about the axle by distance.
const _AXLE=new Vector3(1,0,0),_UP=new Vector3(0,1,0),_qSpin=new Quaternion(),_qSteer=new Quaternion();
const WHEEL_RADIUS=0.65, MAX_STEER=0.5;   // radians of visual lock at full steer
...
export class ScoutDriving {
 ...
 update(dt){
  ...
  this.convoy._ground(v);this._seat();this._wheels(dt);
 }
 // Roll all four wheels by distance travelled and steer the front pair, so the
 // hull no longer slides on locked wheels. Cosmetic: reads the driving state only.
 _wheels(dt){
  const v=this.vehicle;if(!v?.mesh)return;
  let w=v._wheels;
  if(!w){w=v._wheels={fl:v.mesh.getObjectByName('wheels_FL'),fr:v.mesh.getObjectByName('wheels_FR'),
    rl:v.mesh.getObjectByName('wheels_RL'),rr:v.mesh.getObjectByName('wheels_RR')};v._wheelSpin=0;}
  const R=WHEEL_RADIUS*(v.mesh.scale?.x||1);                 // world-space rolling radius
  v._wheelSpin=(v._wheelSpin+(v.speed||0)*dt/Math.max(0.001,R))%(Math.PI*2);
  _qSpin.setFromAxisAngle(_AXLE,v._wheelSpin);               // roll about the axle
  // Front wheels turn with the smoothed steer, same sense the hull turns.
  _qSteer.setFromAxisAngle(_UP,-(v.steerSmooth||0)*MAX_STEER).multiply(_qSpin);
  if(w.fl)w.fl.quaternion.copy(_qSteer);
  if(w.fr)w.fr.quaternion.copy(_qSteer);
  if(w.rl)w.rl.quaternion.copy(_qSpin);
  if(w.rr)w.rr.quaternion.copy(_qSpin);
 }
 exit(force=false){
  ...
  if(v._wheels){for(const k of ['fl','fr','rl','rr'])v._wheels[k]?.quaternion.identity();}   // wheels straight when parked
