...
export class MeleeRecording {
 constructor({seconds=8,hz=30}={}){this.seconds=seconds;this.interval=1/hz;this.frames=[];this.actors=[];this.last=-Infinity;}
 ...
 capture(time){
  ...
  this.last=time;
  const actors=this.actors.map(({fighter:f,nodes})=>({pose:Float32Array.from(nodes.flatMap(n=>[...n.position.toArray(),...n.quaternion.toArray(),...n.scale.toArray(),+n.visible])),hp:f.hp,ki:f.ki,phase:f.mstate||f.grabState||(f.alive?'ready':'ko')}));
  this.frames.push({time,actors});while(this.frames.length>1&&time-this.frames[0].time>this.seconds)this.frames.shift();
 }
 clear(){for(const a of this.actors)a.template?.dispose();this.frames=[];this.actors=[];this.last=-Infinity;}
}
...
export function cloneReviewActor(root){
 ...
 const material=m=>{if(!materials.has(m))materials.set(m,m.clone());return materials.get(m);};
 function copy(n){
...
export function applyReviewPose(nodes,a,b,mix){
 ...
 for(let i=0;i<nodes.length;i++){
  const n=nodes[i],k=i*11;n.position.fromArray(a,k).lerp(v.fromArray(b,k),mix);
