...
import {inventoryLayout,moveInventoryEntry,setHeldStowed,HANDHELD_DEVICE} from './inventory-model.js';
...
export function mountInventory(game){
 ...
 function render(){
  ...
  const titleText=el('div');titleText.append(el('h2',device?HANDHELD_DEVICE.name:'Backpack'),el('small',`${f.def.name} · ${policy.soldier?'Soldier equipment':'Compact field kit'}`));title.append(img,titleText);header.append(title,button('✕',close));dialog.append(header);
  ...
  for(const entry of layout.placed){const item=button(entry.name,()=>{selected=entry.id;if(entry.kind==='device')device=true;render();});item.className='inventory-item';item.style.gridArea=`${entry.y+1}/${entry.x+1}/span ${entry.height}/span ${entry.width}`;item.setAttribute('aria-pressed',String(selected===entry.id));item.draggable=true;item.ondragstart=e=>{selected=entry.id;e.dataTransfer.setData('text/plain',entry.id);};item.append(el('small',entry.kind==='device'?'Coming soon':entry.kind==='stored-weapon'?'Stored':entry.kind==='weapon'?(entry.held?'In hand':'Stowed'):`${entry.item.charges??0} charges`));grid.append(item);}bag.append(grid,el('p','Drag equipment, or select an item then an empty cell. Layout stays with this character for the current life.','inventory-note'));
