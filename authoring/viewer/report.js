...
export function renderReport(m,entry){
 ...
 const clips=(m.clips||[]).map(c=>`<tr><td class="mono">${esc(c.id)}</td><td>${esc(c.take)}</td><td class="mono">${c.duration.toFixed(3)}s</td><td>${c.loop?'loop':'once'}</td><td class="mono">${c.frames}@${c.sampleRate}Hz</td><td>${c.category?`<b>${esc(c.category)}</b> · ${esc(c.posture?.start||'?')} → ${esc(c.posture?.end||'?')}`:'—'}</td><td>${esc(c.handedness)}${c.mirror?' · mirror '+c.mirror:''}</td><td>${(c.events||[]).map(e=>`${esc(e.type)}${e.side?'·'+e.side:''}@${e.t.toFixed(2)}`).join(', ')}</td></tr>`).join('');
 const a=m.acceptance||{visual:'unapproved',blockers:[]};
 const visualLabel={approved:'VISUALLY APPROVED','unapproved':'NOT VISUALLY APPROVED','unapproved-placeholder':'UNAPPROVED PLACEHOLDER ART'}[a.visual]||esc(a.visual);
 const blockers=(a.blockers||[]).map(b=>`<li><b class="fail">BLOCKER</b> <span class="mono">${esc(b.id)}</span> · owner ${esc(b.owner)} — ${esc(b.summary)}</li>`).join('');
 const postureNote=(m.clips||[]).length?(()=>{const prone=m.clips.filter(c=>c.posture?.start==='prone'||c.posture?.end==='prone').map(c=>c.id),supine=m.clips.filter(c=>c.posture?.start==='supine'||c.posture?.end==='supine').map(c=>c.id),getUp=m.clips.filter(c=>c.category==='get-up').map(c=>c.id);return `<p><b>Ground postures (measured):</b> prone ${prone.length?prone.join(', '):'<i>none in this source</i>'} · supine ${supine.length?supine.join(', '):'none'} · get-up ${getUp.length?getUp.join(', ')+' (from '+getUp.map(id=>m.clips.find(c=>c.id===id).posture.start).join('/')+')':'none'}</p>`;})():'';
 ...
 return `
 <p><b class="${entry.ok?'pass':'fail'}">${entry.ok?'STRUCTURAL PASS':'STRUCTURAL FAIL'}</b> · ${entry.errors} validation errors · <b class="${a.visual==='approved'?'pass':'fail'}">${visualLabel}</b>${a.approvedBy?` by ${esc(a.approvedBy)} on ${esc(a.approvedOn)}`:''}${a.note?` — ${esc(a.note)}`:''}</p>
 ${blockers?`<ul class="blockers">${blockers}</ul>`:'<p class="mono" style="font-size:11px">No integration blockers declared.</p>'}
 ${postureNote}
 ...
 ${clips?`<h3>Clips</h3><table class="report"><tr><th>id</th><th>take</th><th>duration</th><th>loop</th><th>frames</th><th>category · posture start → end</th><th>hand</th><th>events</th></tr>${clips}</table>`:''}
